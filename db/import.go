...
func (db *Database) importDoc(docid string, body Body, isDelete bool, existingDoc *sgbucket.BucketDocument, mode ImportMode) (docOut *Document, err error) {
	...
	docOut, _, err = db.updateAndReturnDoc(newDoc.ID, true, existingDoc.Expiry, existingDoc, func(doc *Document) (resultDocument *Document, resultAttachmentData AttachmentData, obsoleteAttachments []string, createNewRevIDSkipped bool, updatedExpiry *uint32, resultErr error) {
		...
		if doc.Cas != existingDoc.Cas {
			...
			if mode == ImportFromFeed {
				return nil, nil, nil, false, nil, base.ErrImportCasFailure
			}
			...
			if mode == ImportOnDemand {
				body = doc.Body()
				if body == nil {
					return nil, nil, nil, false, nil, base.ErrEmptyDocument
				}
				...
				if getExpiryErr != nil {
					return nil, nil, nil, false, nil, getExpiryErr
				}
		...
		if ok || doc.inlineSyncData {
			...
			if migrateErr != nil {
				return nil, nil, nil, false, updatedExpiry, migrateErr
			}
			...
			if !requiresImport {
				...
				return nil, nil, nil, false, updatedExpiry, base.ErrDocumentMigrated
			}
		...
		if doc.Cas == 0 {
			...
			return nil, nil, nil, false, updatedExpiry, base.ErrImportCancelled
		}
		...
		if isSgWrite {
			...
			return nil, nil, nil, false, updatedExpiry, base.ErrAlreadyImported
		}
		...
		if db.DatabaseContext.Options.ImportOptions.ImportFilter != nil {
			...
			if importErr != nil {
				...
				return nil, nil, nil, false, updatedExpiry, base.ErrImportCancelledFilter
			}
			if !shouldImport {
		...
		if shouldGenerateNewRev {
			...
			if err != nil {
				return nil, nil, nil, false, updatedExpiry, err
			}
