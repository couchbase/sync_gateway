...
func TestChannelHistoryPruning(t *testing.T) {
	...
	_, err = rt.WaitForChanges(3, fmt.Sprintf("/db/_changes?since=%v", changes.Last_Seq), "", true)
	assert.NoError(t, err)
	...
	assert.Nil(t, role.ChannelHistory())
}
...
func TestDocChannelSetPruning(t *testing.T) {
	...
	require.Len(t, syncData.ChannelSetHistory, 5)
