...
func TestUserXattrAvoidRevisionIDGeneration(t *testing.T) {
	...
	docRev, err := rt.GetDatabase().GetRevisionCacheForTest().Get(base.TestCtx(t), docKey, syncData.CurrentRev, dataStore.GetCollectionID(), true, false)
	...
	docRev2, err := rt.GetDatabase().GetRevisionCacheForTest().Get(base.TestCtx(t), docKey, syncData.CurrentRev, dataStore.GetCollectionID(), true, false)
	assert.NoError(t, err)
