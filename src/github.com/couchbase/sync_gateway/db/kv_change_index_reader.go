...
func (k *kvChangeIndexReader) initReaderStableClock() (base.SequenceClock, error) {
	k.readerStableSequenceLock.Lock()
	defer k.readerStableSequenceLock.Unlock()

	// Check whether it's been initialized while waiting for the lock
	if k.readerStableSequence != nil {
		return k.readerStableSequence.AsClock(), nil
	} else {
		var err error
		k.readerStableSequence, err = k.loadStableSequence()
		if err != nil {
