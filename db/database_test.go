...
func TestGetRemovedAsUser(t *testing.T) {
	...
	assert.NoError(t, err, "PutWithBody Rev 2")
	...
	assert.NoError(t, err, "PutWithBody Rev 3")
...
func TestNoConflictsMode(t *testing.T) {
	...
	assert.NoError(t, err, "PutWithBody rev after 1-f")
...
// Test tombstoning of existing conflicts after AllowConflicts is set to false via PutWithBody
