...
func (a *AttachmentCompactionManager) Run(ctx context.Context, options map[string]any, persistClusterStatusCallback updateStatusCallbackFunc, terminator *base.SafeTerminator) error {
	...
	case "mark", "":
		...
		worker := func() (shouldRetry bool, err error, value any) {
			...
			var purgeCheckpointsFn base.DCPPurgeCheckpointFunc
			_, a.VBUUIDs, purgeCheckpointsFn, err = attachmentCompactMarkPhase(ctx, dataStore, collectionID, database, a.CompactID, terminator, &a.MarkedAttachments)
			if err != nil {
				shouldRetry, err = a.handleAttachmentCompactionRollbackError(ctx, options, purgeCheckpointsFn, err, MarkPhase)
			}
	...
	case "cleanup":
		...
		worker := func() (shouldRetry bool, err error, value any) {
			persistClusterStatus()
			purgeCheckpointsFn, err := attachmentCompactCleanupPhase(ctx, dataStore, collectionID, database, a.CompactID, a.VBUUIDs, terminator)
			if err != nil {
				shouldRetry, err = a.handleAttachmentCompactionRollbackError(ctx, options, purgeCheckpointsFn, err, CleanupPhase)
			}
			return shouldRetry, err, nil
		}
		// retry loop for handling a rollback during mark phase of compaction process
...
// handleAttachmentCompactionRollbackError checks if the error is a rollback error. If there is any rollback error, reset the checkpoints. If it occurs in the mark phase, restart the the entire process.
func (a *AttachmentCompactionManager) handleAttachmentCompactionRollbackError(ctx context.Context, options map[string]any, purgeCheckpoints base.DCPPurgeCheckpointFunc, err error, phase string) (bool, error) {
	var rollbackErr gocbcore.DCPRollbackError
	if errors.As(err, &rollbackErr) || errors.Is(err, base.ErrVbUUIDMismatch) {
		...
		if purgeCheckpoints != nil {
			purgeCheckpoints(ctx)
		}
		if phase == MarkPhase {
