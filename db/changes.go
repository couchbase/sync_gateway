...
func (db *Database) addDocToChangeEntry(entry *ChangeEntry, options ChangesOptions) {
	...
	if options.IncludeDocs && includeConflicts {
		...
		if err != nil {
			db.Warnf(base.KeyAll, "Changes feed: error getting doc %q: %v", base.UD(entry.ID), err)
			return
		}
		...
		if err != nil {
			db.Warnf(base.KeyAll, "Changes feed: error getting doc sync data %q: %v", base.UD(entry.ID), err)
			return
		}
		db.AddDocInstanceToChangeEntry(entry, doc, options)
		...
		if err != nil {
			db.Warnf(base.KeyAll, "Changes feed: error getting revision body for %q (%s): %v", base.UD(entry.ID), revID, err)
		}
	}
...
func (db *Database) AddDocInstanceToChangeEntry(entry *ChangeEntry, doc *document, options ChangesOptions) {
	...
	if options.IncludeDocs {
		if doc.Body() == nil {
			db.Warnf(base.KeyAll, "AddDocInstanceToChangeEntry called with options.IncludeDocs, but doc is missing Body")
			return
		}
		var err error
		entry.Doc, err = db.getRevFromDoc(doc, revID, false)
		if err != nil {
			db.Warnf(base.KeyAll, "Changes feed: error getting doc %q/%q: %v", base.UD(doc.ID), revID, err)
		}
	}
}
...
func (db *Database) changesFeed(channel string, options ChangesOptions, to string) (<-chan *ChangeEntry, error) {
	...
	go func() {
		...
		for _, logEntry := range log {
			...
			db.Debugf(base.KeyChanges, "Channel feed processing seq:%v in channel %s %s", seqID, base.UD(channel), base.UD(to))
			select {
			case <-options.Terminator:
				db.Debugf(base.KeyChanges, "Terminating channel feed %s", base.UD(to))
				return
			case feed <- &change:
			}
		}
...
func (db *Database) MultiChangesFeed(chans base.Set, options ChangesOptions) (<-chan *ChangeEntry, error) {
	...
	if (options.Continuous || options.Wait) && options.Terminator == nil {
		db.Warnf(base.KeyAll, "MultiChangesFeed: Terminator missing for Continuous/Wait mode")
	}
	if db.SequenceType == IntSequenceType {
		db.Debugf(base.KeyChanges, "Int sequence multi changes feed...")
		return db.SimpleMultiChangesFeed(chans, options)
	} else {
		db.Debugf(base.KeyChanges, "Vector multi changes feed...")
...
func (db *Database) SimpleMultiChangesFeed(chans base.Set, options ChangesOptions) (<-chan *ChangeEntry, error) {
	...
	db.Infof(base.KeyChanges, "MultiChangesFeed(channels: %s, options: %+v) ... %s", base.UD(chans), options, base.UD(to))
	...
	go func() {
		...
		defer func() {
			db.Infof(base.KeyChanges, "MultiChangesFeed done %s", base.UD(to))
			base.StatsExpvars.Add("simpleChanges_active", -1)
			close(output)
		}()
		...
		for {
			...
			for name, vbSeqAddedAt := range channelsSince {
				...
				if seqAddedAt > currentCachedSequence {
					db.Debugf(base.KeyChanges, "Grant for channel [%s] is after the current sequence - skipped for this iteration.  Grant:[%d] Current:[%d] %s", base.UD(name), seqAddedAt, currentCachedSequence, base.UD(to))
				...
				if options.Continuous {
					...
					if lateSequenceFeedHandler != nil {
						...
						if err != nil {
							db.Warnf(base.KeyAll, "MultiChangesFeed got error reading late sequence feed %q: %v", base.UD(name), err)
						} else {
			...
			for {
				...
				if currentCachedSequence < minEntry.Seq.Seq {
					db.Debugf(base.KeyChanges, "Found sequence later than stable sequence: stable:[%d] entry:[%d] (%s)", currentCachedSequence, minEntry.Seq.Seq, base.UD(minEntry.ID))
				...
				db.Debugf(base.KeyChanges, "MultiChangesFeed sending %+v %s", base.UD(minEntry), base.UD(to))
...
func (context *DatabaseContext) WaitForSequence(sequence uint64) (err error) {
	context.Debugf(base.KeyChanges, "Waiting for sequence: %d", sequence)
	if err == nil {
...
func (context *DatabaseContext) WaitForPendingChanges() (err error) {
	...
	context.Debugf(base.KeyChanges, "Waiting for sequence: %d", lastSequence)
	if err == nil {
...
func (db *Database) DocIDChangesFeed(userChannels base.Set, explicitDocIds []string, options ChangesOptions) (<-chan *ChangeEntry, error) {
	...
	createChangeEntry := func(docid string) *ChangeEntry {
		...
		if err != nil {
			db.Infof(base.KeyChanges, "Unable to get changes for docID %v, caused by %v", base.UD(docid), err)
			return nil
		}
