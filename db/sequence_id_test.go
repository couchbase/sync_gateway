...
// TestSequenceIDRawString asserts that RawString always includes LowSeq and TriggeredBy when they're
// non-zero, reconstructing the wire form the client sent even in the three cases where String() omits a
// component - see intSeqToString.
func TestSequenceIDRawString(t *testing.T) {
	testCases := []struct {
		name      string
		seq       SequenceID
		seqStr    string
		...
		{
			name:      "simple sequence",
			seq:       SequenceID{Seq: 5},
			seqStr:    "5",
			rawSeqStr: "5",
		},
		{
			name:      "lowSeq above seq is omitted from String, non-backfill",
			seq:       SequenceID{LowSeq: 5, Seq: 3},
			seqStr:    "3",
			rawSeqStr: "5::3",
		},
		{
			name:      "lowSeq above triggeredBy is omitted from String, backfill",
			seq:       SequenceID{LowSeq: 5, TriggeredBy: 4, Seq: 2},
			seqStr:    "4:2",
			rawSeqStr: "5:4:2",
		},
		{
			name:      "triggeredBy not above seq is omitted from String",
			seq:       SequenceID{TriggeredBy: 10, Seq: 20},
			seqStr:    "20",
			rawSeqStr: "10:20",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.seqStr, tc.seq.String())
			assert.Equal(t, tc.rawSeqStr, tc.seq.RawString())
		})
	}
}
