	...
	go func() {
		...
		for {
			...
			case event := <-w.eventFeed:
				...
				case mutationEvent:
					InfofCtx(ctx, KeyDCP, "CBG-4640 DEBUG DCPWorker.mutationEvent: key=%s vbID=%d seq=%d valueLen=%d datatype=%d", UD(string(e.key)), vbID, e.seq, len(e.value), e.datatype)
					if w.mutationCallback != nil {
						w.mutationCallback(e.asFeedEvent())
					}
					w.updateSeq(ctx, e.key, vbID, e.seq)
				case deletionEvent:
					InfofCtx(ctx, KeyDCP, "CBG-4640 DEBUG DCPWorker.deletionEvent: key=%s vbID=%d seq=%d valueLen=%d datatype=%d", UD(string(e.key)), vbID, e.seq, len(e.value), e.datatype)
					if w.mutationCallback != nil && !w.ignoreDeletes {
						w.mutationCallback(e.asFeedEvent())
					}
					w.updateSeq(ctx, e.key, vbID, e.seq)
				case seqnoAdvancedEvent:
					InfofCtx(ctx, KeyDCP, "CBG-4640 DEBUG DCPWorker.seqnoAdvancedEvent: vbID=%d seq=%d", vbID, e.seq)
