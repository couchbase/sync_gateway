...
# Build Sync Gateway with the right edition
export SG_EDITION="${SG_EDITION}"

if [ ${USE_GO_MODULES} == "true" ]; then
  cd sgw
else
  rm -rf sgw
  mkdir sgw 
  ...
  cd "$WORKSPACE/sgw/godeps/src/github.com/couchbase/sync_gateway"
...
export INT_LOG_FILE_NAME="int_logs_${SG_EDITION}_${SG_COMMIT}_${TARGET_TEST}"
...
go test ${GO_TEST_FLAGS} -coverprofile=coverage_int.out -coverpkg=github.com/couchbase/sync_gateway/${TARGET_PACKAGE} github.com/couchbase/sync_gateway/${TARGET_PACKAGE} 2>&1 | tee "${INT_LOG_FILE_NAME}.txt" | grep -E '(--- (FAIL|PASS|SKIP):|github.com/couchbase/sync_gateway(/.+)?\t|TEST: |panic: )'
...
~/go/bin/go2xunit -input "${INT_LOG_FILE_NAME}.txt" -output "${INT_LOG_FILE_NAME}.xml"
...
if [ "${TEST_FAILED}" = true ] ; then
  ...
  if grep -q 'FAIL:' "${INT_LOG_FILE_NAME}.txt"; then
