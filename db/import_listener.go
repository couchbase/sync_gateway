...
func (il *importListener) StartImportFeed(dbContext *DatabaseContext) (err error) {
	...
	if dbContext.OnlyDefaultCollection() {
		il.importDestKey = base.DestKey(il.dbName, "", []string{}, base.ImportDestType)
	} else {
		il.importDestKey = base.DestKey(il.dbName, scopeName, collectionNamesByScope[scopeName], base.ImportDestType)
	}
	...
	il.cbgtContext, err = base.StartShardedDCPFeed(il.loggingCtx, dbContext.Name, dbContext.Options.GroupID, dbContext.UUID, dbContext.Heartbeater,
		il.bucket, cbStore.GetSpec(), scopeName, collectionNamesByScope[scopeName], dbContext.Options.ImportOptions.ImportPartitions, dbContext.CfgSG, false)
	return err
}
