...
func TestChannelCacheMaxSize(t *testing.T) {
	...
	cache := database.ChangeCacheForTest(t).GetChannelCacheForTest(t)

	collectionID := db.GetSingleDatabaseCollection(t, database.DatabaseContext).GetCollectionID()

	// Make channels active
	_, err := cache.GetChanges(ctx, channels.NewID("TestA", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)
	_, err = cache.GetChanges(ctx, channels.NewID("TestB", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)
	_, err = cache.GetChanges(ctx, channels.NewID("TestC", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)
	_, err = cache.GetChanges(ctx, channels.NewID("TestD", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)

	// Add some entries to caches, leaving some empty caches
	cache.AddToCache(ctx, db.MakeLogEntry(1, "doc1", "1-a", []string{"TestB", "TestC", "TestD"}, collectionID))
	cache.AddToCache(ctx, db.MakeLogEntry(2, "doc2", "1-a", []string{"TestB", "TestC", "TestD"}, collectionID))
	cache.AddToCache(ctx, db.MakeLogEntry(3, "doc3", "1-a", []string{"TestB", "TestC", "TestD"}, collectionID))
	cache.AddToCache(ctx, db.MakeLogEntry(4, "doc4", "1-a", []string{"TestC"}, collectionID))

	database.UpdateCalculatedStats(ctx)

	maxEntries := database.DbStats.Cache().ChannelCacheMaxEntries.Value()
...
func TestChannelCacheCurrentVersion(t *testing.T) {
	...
	// Make channels active
	_, err := cache.GetChanges(ctx, channels.NewID("chanA", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)
	_, err = cache.GetChanges(ctx, channels.NewID("chanB", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)
	_, err = cache.GetChanges(ctx, channels.NewID("chanC", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)
	_, err = cache.GetChanges(ctx, channels.NewID("chanD", collectionID), db.GetChangesOptionsWithCtxOnly(t))
	require.NoError(t, err)

	cache.AddToCache(ctx, db.MakeTestLogEntryWithCV(1, "doc1", "1-a", []string{"chanB", "chanC", "chanD"}, collectionID, "test1", 123))
	cache.AddToCache(ctx, db.MakeTestLogEntryWithCV(2, "doc2", "1-a", []string{"chanB", "chanC", "chanD"}, collectionID, "test2", 1234))
	cache.AddToCache(ctx, db.MakeTestLogEntryWithCV(3, "doc3", "1-a", []string{"chanC", "chanD"}, collectionID, "test3", 12345))
	cache.AddToCache(ctx, db.MakeTestLogEntryWithCV(4, "doc4", "1-a", []string{"chanC"}, collectionID, "test4", 123456))

	// assert on channel cache entries for 'chanC'
	entriesChanC, err := cache.GetChanges(ctx, channels.NewID("chanC", collectionID), db.GetChangesOptionsWithZeroSeq(t))
	...
	entriesChanD, err := cache.GetChanges(ctx, channels.NewID("chanD", collectionID), db.GetChangesOptionsWithZeroSeq(t))
	assert.NoError(t, err)
	...
	entriesChanB, err := cache.GetChanges(ctx, channels.NewID("chanB", collectionID), db.GetChangesOptionsWithZeroSeq(t))
	assert.NoError(t, err)
...
func TestChannelCacheSimpleCompact(t *testing.T) {
	...
	ctx := base.TestCtx(t)
	cache, err := db.NewChannelCacheForTest(t, base.TestCtx(t), "testDb", options, db.QueryHandlerFactoryForTest, activeChannels, testStats)
	...
	for i := 1; i <= 16; i++ {
		...
		cache.AddChannelCacheForTest(t, ctx, channels.NewID(channelName, base.DefaultCollectionID))
	}
	// Validate cache size
	assert.Equal(t, 16, cache.ChannelCachesForTest(t).Length())
...
func TestChannelCacheCompactInactiveChannels(t *testing.T) {
	...
	for i := 1; i <= 19; i++ {
		channel := channels.NewID(fmt.Sprintf("chan_%d", i), base.DefaultCollectionID)
		_, isCached := cache.ChannelCachesForTest(t).Get(channel)
		if i%2 == 1 {
...
func TestChannelCacheCompactNRU(t *testing.T) {
	...
	for i := 1; i <= 5; i++ {
		channel := channels.NewID(fmt.Sprintf("chan_%d", i), base.DefaultCollectionID)
		cacheElement, isCached := cache.ChannelCachesForTest(t).Get(channel)
		assert.True(t, isCached, fmt.Sprintf("Expected %s to be cached during recently used update", channel))
		db.SetRecentlyUsedForTest(t, db.AsSingleChannelCache(ctx, cacheElement), true)
	}
...
func TestChannelCacheHighLoadCacheHit(t *testing.T) {
	...
	activeChannels := channels.NewActiveChannels(activeChannelStat)
	ctx := base.TestCtx(t)
	cache, err := db.NewChannelCacheForTest(t, ctx, "testDb", options, queryHandler.AsFactory, activeChannels, testStats)
	require.NoError(t, err, "Background task error whilst creating channel cache")
	defer cache.Stop(ctx)
	...
	queryEntry := db.MakeTestLogEntryForChannels(1, channelNames)
	...
	logEntry := db.MakeTestLogEntryForChannels(1, channelNames)
	...
	for range workerCount {
		...
		go func() {
			...
			for range getChangesCount {
				...
				options := db.GetChangesOptionsWithCtxOnly(t)
...
func TestChannelCacheBackgroundTaskWithIllegalTimeInterval(t *testing.T) {
	...
	cache, err := db.NewChannelCacheForTest(t, base.TestCtx(t), "testDb", options, queryHandler.AsFactory, activeChannels, testStats)
	assert.Error(t, err, "Background task error whilst creating channel cache")
	assert.Nil(t, cache)

	backgroundTaskError, ok := err.(*db.BackgroundTaskError)
	require.True(t, ok)
...
func TestChannelCacheActiveOnlyAndLimit(t *testing.T) {
	ctx, database, collection := setupDBWithChannelCacheSize(t, 2)
	...
	revID, _, err := collection.Put(ctx, doc1, db.Body{"channels": activeChannel})
	require.NoError(t, err)
	_, _, err = collection.Put(ctx, doc1, db.Body{"channels": inactiveChannel, "_rev": revID})
	require.NoError(t, err)
	revID, _, err = collection.Put(ctx, doc2, db.Body{"channels": activeChannel})
	require.NoError(t, err)
	_, _, err = collection.Put(ctx, doc2, db.Body{"channels": inactiveChannel, "_rev": revID})
	require.NoError(t, err)

	_, _, err = collection.Put(ctx, doc3, db.Body{"channels": activeChannel})
	require.NoError(t, err)
	...
	require.Len(t, db.GetChangesForTest(t, collection, base.SetOf(activeChannel), changesOptions), 3)
	...
	for _, limit := range []int{0, 1} {
		t.Run("limit="+fmt.Sprint(limit), func(t *testing.T) {
			...
			require.Len(t, db.GetChangesForTest(t, collection, base.SetOf(activeChannel), changesOptions), 1)
		})
	}
}
...
func TestChannelCacheActiveOnlyScenarios(t *testing.T) {
	...
	t.Run("query returns an active rev, cache is all removals", func(t *testing.T) {
		...
		_, _, err := collection.Put(ctx, "doc1", db.Body{"channels": activeChannel})
		require.NoError(t, err)

		// doc2: active (seq 2) -> inactive (seq 3) - seq 3 in cache
		revID2, _, err := collection.Put(ctx, "doc2", db.Body{"channels": activeChannel})
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc2", db.Body{"channels": "other", "_rev": revID2})
		require.NoError(t, err)

		// doc3: active (seq 4) -> inactive (seq 5) - seq 5 in cache
		revID3, _, err := collection.Put(ctx, "doc3", db.Body{"channels": activeChannel})
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc3", db.Body{"channels": "other", "_rev": revID3})
		require.NoError(t, err)

		database.WaitForPendingChanges(t)

		// With limit 1 (before)
		changesOptions := db.ChangesOptions{Since: db.SequenceID{Seq: 0}, ActiveOnly: true, Limit: 1, ChangesCtx: base.TestCtx(t)}
		changes := db.GetChangesForTest(t, collection, base.SetOf(activeChannel), changesOptions)
	...
	t.Run("query has no active revs, cache has no active revs", func(t *testing.T) {
		...
		revID1, _, err := collection.Put(ctx, "doc1", db.Body{"channels": activeChannel})
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc1", db.Body{"channels": "other", "_rev": revID1})
		require.NoError(t, err)
	...
	t.Run("cache populated, query requires pagination", func(t *testing.T) {
		...
		changesOptions := db.ChangesOptions{Since: db.SequenceID{Seq: 0}, ChangesCtx: base.TestCtx(t)}
		_ = db.GetChangesForTest(t, collection, base.SetOf(activeChannel), changesOptions)
		// Write 20 docs to the channel.  5 should be cached, 15 require query
		for i := range 20 {
			docID := fmt.Sprintf("doc%d", i+1)
			_, _, err := collection.Put(ctx, docID, db.Body{"channels": activeChannel})
			require.NoError(t, err)
		}
		database.WaitForPendingChanges(t)
		changesOptions = db.ChangesOptions{Since: db.SequenceID{Seq: 0}, ActiveOnly: true, Limit: 0, ChangesCtx: base.TestCtx(t)}
		changes := db.GetChangesForTest(t, collection, base.SetOf(activeChannel), changesOptions)
	...
	t.Run("cache populated, query requires pagination with limit", func(t *testing.T) {
		...
		_ = db.GetChangesForTest(t, collection, base.SetOf(activeChannel), changesOptions)
		for i := 1; i <= 20; i++ {
			_, _, err := collection.Put(ctx, fmt.Sprintf("doc%d", i), db.Body{"channels": activeChannel})
			require.NoError(t, err)
		}
		database.WaitForPendingChanges(t)
		...
		changesOptions = db.ChangesOptions{Since: db.SequenceID{Seq: 0}, ActiveOnly: true, Limit: 10, ChangesCtx: base.TestCtx(t)}
		changes := db.GetChangesForTest(t, collection, base.SetOf(activeChannel), changesOptions)
...
func setupDBWithChannelCacheSize(t *testing.T, maxLength int) (context.Context, *db.Database, *db.DatabaseCollectionWithUser) {
	cacheOptions := db.DefaultCacheOptions()
	cacheOptions.ChannelCacheMaxLength = maxLength
	return db.SetupDBWithChannelCacheSettings(t, cacheOptions)
}
...
func FuzzChannelCacheActiveOnly(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, numDocs, cacheMaxLength, queryLimit, requestLimit uint8) {
		...
		_ = db.GetChangesForTest(t, collection, base.SetOf("active"), db.ChangesOptions{
			Since: db.SequenceID{Seq: 0}, ChangesCtx: base.TestCtx(t),
		})
		...
		for i := 1; i <= n; i++ {
			...
			case 0:
				...
				nextDocID++
				body = db.Body{"channels": "active"}
				active = true
			case 1:
				docID = fmt.Sprintf("doc_oth_%d", nextDocID)
				nextDocID++
				body = db.Body{"channels": "other"}
				active = false
			case 2:
				...
				docID = docIDs[idx]
				state := docsMap[docID]
				body = db.Body{"channels": "active", "_rev": state.revID}
				active = true
			case 3:
				...
				docID = docIDs[idx]
				state := docsMap[docID]
				body = db.Body{"channels": "other", "_rev": state.revID}
				active = false
			}
		...
		changes := db.GetChangesForTest(t, collection, base.SetOf("active"), db.ChangesOptions{
			Since:      db.SequenceID{Seq: 0},
			ActiveOnly: true,
...
func TestChannelCacheActiveOnlyLimitWithCrossChannelGap(t *testing.T) {
	...
	_ = db.GetChangesForTest(t, collection, base.SetOf(activeChannel), primingOptions)

	// doc1: active (seq1) -> removed from channel (seq2)
	revID, _, err := collection.Put(ctx, "doc1", db.Body{"channels": activeChannel})
	require.NoError(t, err)
	_, _, err = collection.Put(ctx, "doc1", db.Body{"channels": otherChannel, "_rev": revID})
	require.NoError(t, err)

	// doc2: active (seq3) -> removed from channel (seq4)
	revID, _, err = collection.Put(ctx, "doc2", db.Body{"channels": activeChannel})
	require.NoError(t, err)
	_, _, err = collection.Put(ctx, "doc2", db.Body{"channels": otherChannel, "_rev": revID})
	require.NoError(t, err)

	// docGap: seq5, entirely in a different channel - creates a sequence gap for activeChannel.
	_, _, err = collection.Put(ctx, "docGap", db.Body{"channels": otherChannel})
	require.NoError(t, err)
	...
	_, _, err = collection.Put(ctx, "doc3", db.Body{"channels": activeChannel})
	require.NoError(t, err)
	_, _, err = collection.Put(ctx, "doc4", db.Body{"channels": activeChannel})
	require.NoError(t, err)
...
func TestChannelCacheActiveOnlyBoundariesAndGaps(t *testing.T) {
	...
	t.Run("Query and cache both exactly at boundary (No Gap)", func(t *testing.T) {
		...
		// Prime cache
		_ = db.GetChangesForTest(t, collection, base.SetOf(activeChannel), db.ChangesOptions{
			Since: db.SequenceID{Seq: 0}, ChangesCtx: base.TestCtx(t),
		})

		_, _, err := collection.Put(ctx, "doc1", db.Body{"channels": activeChannel})
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc2", db.Body{"channels": activeChannel})
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc3", db.Body{"channels": activeChannel})
		require.NoError(t, err)
		...
		changes := db.GetChangesForTest(t, collection, base.SetOf(activeChannel), db.ChangesOptions{
			Since:      db.SequenceID{Seq: 0},
			ActiveOnly: true,
			Limit:      10,
			ChangesCtx: base.TestCtx(t),
	...
	t.Run("Query gap before boundary, cache at boundary", func(t *testing.T) {
		...
		_, _, err := collection.Put(ctx, "doc1", db.Body{"channels": activeChannel}) // Seq 1
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "docGap", db.Body{"channels": otherChannel}) // Seq 2
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc2", db.Body{"channels": activeChannel}) // Seq 3
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc3", db.Body{"channels": activeChannel}) // Seq 4
		require.NoError(t, err)
	...
	t.Run("Query at boundary, cache gap after boundary", func(t *testing.T) {
		...
		_, _, err := collection.Put(ctx, "doc1", db.Body{"channels": activeChannel}) // Seq 1
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc2", db.Body{"channels": activeChannel}) // Seq 2
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "docGap", db.Body{"channels": otherChannel}) // Seq 3
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc3", db.Body{"channels": activeChannel}) // Seq 4
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc4", db.Body{"channels": activeChannel}) // Seq 5
		require.NoError(t, err)
		...
		changes10 := db.GetChangesForTest(t, collection, base.SetOf(activeChannel), db.ChangesOptions{
			Since:      db.SequenceID{Seq: 0},
			ActiveOnly: true,
			Limit:      10,
			ChangesCtx: base.TestCtx(t),
	...
	t.Run("Gaps in both directions", func(t *testing.T) {
		...
		_, _, err := collection.Put(ctx, "doc1", db.Body{"channels": activeChannel}) // Seq 1
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "docGap1", db.Body{"channels": otherChannel}) // Seq 2
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc2", db.Body{"channels": activeChannel}) // Seq 3
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "docGap2", db.Body{"channels": otherChannel}) // Seq 4
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc3", db.Body{"channels": activeChannel}) // Seq 5
		require.NoError(t, err)
		_, _, err = collection.Put(ctx, "doc4", db.Body{"channels": activeChannel}) // Seq 6
		require.NoError(t, err)
