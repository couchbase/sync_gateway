		...
		for _, testCase := range testCases {
			t.Run(testCase.name, func(t *testing.T) {
				...
				if testCase.newCVGenerated {
					...
					// The merge current version is generated by the SG HLC rather than the CAS macro, so it is
					// independent of - and must not exceed - the document CAS. Assert that invariant, then copy
					// the actual value across so the struct comparison validates the remaining fields exactly.
					require.NotZero(t, resolvedDoc.HLV.Version)
					require.LessOrEqual(t, resolvedDoc.HLV.Version, resolvedDoc.Cas)
