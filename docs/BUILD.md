...
_Note:_ if you want to run the bootstrap initialization and start on a particular Sync Gateway commit, you can provide the `-c` flag, eg `./bootstrap.sh -c y0pl33g0r425`.
...
| Name                         | Description                                                                                   |
| ---------------------------- | --------------------------------------------------------------------------------------------- |
| SG_TEST_BACKING_STORE        | Walrus by default, but can set to "Couchbase" to have it use <http://127.0.0.1:8091>            |
| SG_TEST_COUCHBASE_SERVER_URL | Uses <http://127.0.0.1:8091> by default, but you can set this to any valid Couchbase Server URL |
