...
func GetCouchbaseBucketGoCB(spec BucketSpec) (bucket *CouchbaseBucketGoCB, err error) {
	...
	if err != nil {
		return nil, pkgerrors.WithStack(NewRedactableError("Error opening GoCB bucket. Bucket: %s.  Error: %v", MD(spec.BucketName), err))
	}
...
func (bucket *CouchbaseBucketGoCB) Get(k string, rv interface{}) (cas uint64, err error) {
	...
	if !ok {
		return 0, NewRedactableError("Get: Error doing type assertion of %v into a uint64,  Key: %v", result, UD(k))
	}

	if err != nil {
		err = pkgerrors.WithStack(NewRedactableError("Unrecoverable GoCB error doing Get(). Doc ID: %s.  Error: %v.", UD(k), err))
	}
...
func (bucket *CouchbaseBucketGoCB) AddRaw(k string, exp uint32, v []byte) (added bool, err error) {
	...
	if err != nil {
		...
		err = pkgerrors.WithStack(NewRedactableError("Error calling GoCB AddRaw(). Doc ID: %s.  Error: %v.", UD(k), err))
	}
...
func (bucket *CouchbaseBucketGoCB) Set(k string, exp uint32, v interface{}) error {
	...
	if err != nil {
		err = pkgerrors.WithStack(NewRedactableError("Error calling GoCB Set(). Doc ID: %s. Error: %v.", UD(k), err))
	}
	return err
...
func (bucket *CouchbaseBucketGoCB) Incr(k string, amt, def uint64, exp uint32) (uint64, error) {
	...
	cas, ok := result.(uint64)
	if !ok {
		return 0, NewRedactableError("Incr: Error doing type assertion of %v into a uint64,  Key: %v", result, UD(k))
	}

	if err != nil {
		err = pkgerrors.WithStack(NewRedactableError("Unrecoverable GoCB error doing Incr().  Doc ID: %s %s with amt: %d.  Error: %v", UD(k), amt, err))
	}
...
func (bucket *CouchbaseBucketGoCB) getExpirySingleAttempt(k string) (expiry uint32, getMetaError error) {
	...
	if getMetaError != nil {
		getMetaError = pkgerrors.WithStack(NewRedactableError("Error getting expiry value. Doc ID: %s.   Error: %v", UD(k), getMetaError))
	}
