...
func TestPushUnknownAttachmentAsStub(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		length, digest := btcRunner.saveAttachment(btc.id, contentType, attachmentAData)
...
func TestAttachmentWithErroneousRevPos(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		_ = btcRunner.PushRevWithHistory(btc.id, docID, &version, []byte(`{"_attachments": {"hello.txt": {"revpos":1,"stub":true,"length": 19,"digest":"sha1-l+N7VpXGnoxMm8xfvtWPbz2YvDc="}}}`), 1, 0)
