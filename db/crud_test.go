...
func TestRepairFailureDoesNotLeakSequence(t *testing.T) {
	...
	lb := base.NewLeakyBucket(tb, base.LeakyBucketConfig{
		...
		UpdateXattrsCallback: func(key string) {
			...
			raceOnce.Do(func() {
				var body []byte
				_, err := tb.GetSingleDataStore().Get(key, &body)
				require.NoError(t, err)
				require.NoError(t, tb.GetSingleDataStore().Set(key, 0, nil, body))
			})
		},
		IgnoreClose: true,
	})
	...
	assert.Equal(t, int64(1), releasedCount, "sequence %d was allocated and never released", abandonedSequence)
	...
	exists, err := dbCtx.MetadataStore.Exists(unusedSeqKey)
	require.NoError(t, err)
...
func TestWritePathRepairMouChain(t *testing.T) {
	...
	require.NoError(t, collection.dataStore.Set(docID, 0, nil, []byte(`{"sdk":true}`)))
	_, sdkWriteCas, err := collection.dataStore.GetRaw(docID)
	require.NoError(t, err)
	dbCtx.FlushRevisionCacheForTest()
	...
	_, casAfterRepair, err := collection.dataStore.GetRaw(docID)
	require.NoError(t, err)
