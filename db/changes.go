...
func (db *Database) DocIDChangesFeed(userChannels base.Set, explicitDocIds []string, options ChangesOptions) (<-chan *ChangeEntry, error) {
	...
	for _, docID := range explicitDocIds {
		row := db.createChangeEntry(docID, options)
		if row != nil {
...
// Generate a change entry for given docid
func (db *Database) createChangeEntry(docid string, options ChangesOptions) *ChangeEntry {
