...
func (h *handler) handleAllDocs() error {
	...
	if h.user != nil {
		var err error
		availableChannels, err = h.user.InheritedCollectionChannels(h.collection.ScopeName, h.collection.Name)
		if err != nil {
			return err
		}
		if availableChannels == nil {
			base.AssertfCtx(h.ctx(), "User %q has no channels in handleAllDocs", base.UD(h.user.Name()))
			return base.HTTPErrorf(http.StatusInternalServerError, "user has no channels")
		}
