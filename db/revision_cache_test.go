...
func TestLRURevisionCacheEviction(t *testing.T) {
	...
	for docID := 0; docID < 10; docID++ {
		...
		cache.Put(DocumentRevision{DocID: id, Body: []byte{}, RevID: "1-abc", History: Revisions{"start": 1}})
	}
	...
	for i := 10; i < 13; i++ {
		docID := strconv.Itoa(i)
		cache.Put(DocumentRevision{DocID: docID, Body: []byte{}, RevID: "1-abc", History: Revisions{"start": 1}})
	}
...
func TestPutRevisionCacheAttachmentProperty(t *testing.T) {
	...
	assert.True(t, ok)
	var docBody Body
	assert.NoError(t, docBody.Unmarshal(docRevision.Body))
	_, ok = docBody[BodyAttachments]
