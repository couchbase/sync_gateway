...
func (auth *Authenticator) rebuildChannels(princ Principal) error {
	...
	base.LogToR("Access", "Computed channels for %q: %s",
		base.UD(princ.Name()), base.UD(channels))
...
func (auth *Authenticator) rebuildRoles(user User) error {
	...
	base.LogToR("Access", "Computed roles for %q: %s",
		base.UD(user.Name()), base.UD(roles))
...
func (auth *Authenticator) authenticateJWT(jwt jose.JWT, provider *OIDCProvider) (User, jose.JWT, error) {
	...
	if user != nil && identity.Email != "" {
		if identity.Email != user.Email() {
			base.LogToR("OIDC+", "Updating user email to: %s", base.UD(identity.Email))
	...
	if user == nil && provider.Register {
		base.LogToR("OIDC+", "Registering new user: %s with email: %s",
			base.UD(username), base.UD(identity.Email))
		var err error
