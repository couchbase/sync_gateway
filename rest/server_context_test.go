...
func TestGetOrAddDatabaseFromConfig(t *testing.T) {
	...
	dbContext, err := serverContext._getOrAddDatabaseFromConfig(DatabaseConfig{DbConfig: dbConfig}, false, db.ConnectToBucket)
	...
	dbContext, err = serverContext._getOrAddDatabaseFromConfig(DatabaseConfig{DbConfig: dbConfig}, true, db.ConnectToBucket)
