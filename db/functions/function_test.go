...
func testUserFunctionsCommon(t *testing.T, ctx context.Context, db *db.Database) {
	...
	assertErrorContains(t, err, "oops")
	assertErrorContains(t, err, "exceptional")
	jserr := err.(*jsError)
	assert.NotNil(t, jserr)

	// Call depth limit:
	_, err = db.CallUserFunction("factorial", map[string]any{"n": kUserFunctionMaxCallDepth + 1}, true, ctx)
	assertErrorContains(t, err, "User function recursion too deep")
	assertErrorContains(t, err, "factorial")
}
...
func testUserFunctionsAsAdmin(t *testing.T, ctx context.Context, db *db.Database) {
	...
	assertErrorContains(t, err, "No user")
	jserr := err.(*jsError)
	assert.NotNil(t, jserr)
...
func testUserFunctionsAsUser(t *testing.T, ctx context.Context, db *db.Database) {
	...
	assertHTTPError(t, err, 403)
	assertErrorContains(t, err, "great_and_terrible") // failed fn name should appear in error
...
func TestUserFunctionsMaxFunctionCount(t *testing.T) {
	...
	assertErrorContains(t, err, "too many functions declared (> 1)")
}
...
func assertHTTPError(t *testing.T, err error, status int) bool {
	...
	return assert.Error(t, err) &&
		assert.Truef(t, errors.As(err, &httpErr), "error is not an HTTPError, rather %#v", err) &&
		assert.Equal(t, status, httpErr.Status, "Error is: %#v", err)
}

// polyfill for assert.ErrorContains()
func assertErrorContains(t *testing.T, theError error, contains string, msgAndArgs ...interface{}) bool {
	...
	if !strings.Contains(actual, contains) {
		return assert.Fail(t, fmt.Sprintf("Error %#v does not contain %#v", actual, contains), msgAndArgs...)
	}
...
func setupTestDBForBucketWithOptions(t testing.TB, tBucket base.Bucket, dbcOptions db.DatabaseContextOptions) (*db.Database, context.Context) {
	...
	dbCtx, err := db.NewDatabaseContext("db", tBucket, false, dbcOptions)
