...
func TestBlipGetCollections(t *testing.T) {
	...
	checkpointBody := db.Body{}
	checkpointID1 := "checkpoint1"
	dbInstance := db.Database{DatabaseContext: rt.GetDatabase()}
	revID, err := dbInstance.PutSpecial(db.DocTypeLocal, db.CheckpointDocIDPrefix+checkpointID1, checkpointBody)
	require.NoError(t, err)
