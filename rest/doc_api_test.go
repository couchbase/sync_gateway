...
func TestDocumentNumbers(t *testing.T) {
	...
	for _, test := range tests {
		rt.Run(test.name, func(ts *testing.T) {
			...
			assert.True(ts, HasActiveChannel(rawResponse.Xattrs.Sync.Channels, test.expectedFormatChannel), fmt.Sprintf("Expected channel %s was not found in document channels (%s)", test.expectedFormatChannel, test.name))
