...
func TestUserGraphQLWithN1QL(t *testing.T) {
	...
	_, _, _ = db.Put("a", Body{"type": "task", "title": "Applesauce", "done": true, "tags": []string{"fruit", "soft"}, "channels": "wonderland"})
	_, _, _ = db.Put("b", Body{"type": "task", "title": "Beer", "description": "Bass ale please", "channels": "wonderland"})
	_, _, _ = db.Put("m", Body{"type": "task", "title": "Mangoes", "channels": "wonderland"})
...
func TestGraphQLMissingFieldResolvers(t *testing.T) {
	...
	assertErrorContains(t, err, "Query.scandals: Missing")
	assert.NotContains(t, err.Error(), "Query.secrets")
	assertErrorContains(t, err, "Mutation.crush: Missing")
	assertErrorContains(t, err, "Mutation.mangle: Missing")
	assertErrorContains(t, err, "Mutation.destroy: Missing")
}
...
func TestGraphQLMissingTypeResolvers(t *testing.T) {
	...
	_, err := CompileGraphQL(&config)
	assertErrorContains(t, err, "Location: GraphQL interface must implement a '__typename' resolver")
...
func TestGraphQLMaxSchemaSize(t *testing.T) {
	...
	_, err := CompileGraphQL(&config)
	assertErrorContains(t, err, "GraphQL schema too large (> 20 bytes)")
}
...
func TestGraphQLMaxResolverCount(t *testing.T) {
	...
	_, err := CompileGraphQL(&config)
	assertErrorContains(t, err, "too many GraphQL resolvers (> 1)")
}
...
func TestGraphQLSubgraph(t *testing.T) {
	...
	db, _ := setupTestDBWithFunctions(t, nil, &kTestGraphQLConfigWithN1QL)
	defer db.Close()
