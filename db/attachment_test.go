...
func TestBackupOldRevisionWithAttachments(t *testing.T) {
	...
	rev1OldBody, err := db.getOldRevisionJSON(db.Ctx, docID, rev1ID)
	...
	rev2OldBody, err := db.getOldRevisionJSON(db.Ctx, docID, rev2ID)
	if deltasEnabled && xattrsEnabled {
		require.NoError(t, err)
...
func TestMigrateBodyAttachments(t *testing.T) {
  ...
  "channels": {
    "ABC": null
		...
		docSyncData, err := db.GetDocSyncData(db.Ctx, docKey)
		assert.NoError(t, err)
	...
	t.Run("2.1 meta, read active rev", func(t *testing.T) {
		...
		syncData, err := db.GetDocSyncData(db.Ctx, docKey)
		assert.NoError(t, err)
