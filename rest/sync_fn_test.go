...
func setUpPromotedLiveBranch(t *testing.T, rt *RestTester, docID string) (version1a, version2a, version2b DocVersion, bodyKey string) {
	...
	rt.GetDatabase().Options.AllowConflicts = base.Ptr(true)

	// Non-winning bodies are only moved out to a _sync:rb: doc when xattrs are enabled, so without
	// this the padded bodies below stay inline and the externalised-body path under test is never
	// exercised on a SG_TEST_USE_XATTRS=false run. Forcing it keeps both modes on the same path.
	base.TestExternalRevStorage = true
	t.Cleanup(func() { base.TestExternalRevStorage = false })
	...
	require.Equal(t, version2b.RevID, doc.CurrentRev, "branch b should be the winning revision")
	require.Empty(t, doc.History[version2a.RevID].Body, "branch a's body should not be inline in the rev tree")
	bodyKey = doc.History[version2a.RevID].BodyKey
...
func TestSyncFnLiveBranchPromotedWithUnreadableBody(t *testing.T) {
	...
	response := rt.SendAdminRequest(http.MethodDelete,
		fmt.Sprintf("/{{.keyspace}}/%s?rev=%s", testDocID, version2b.RevID), "")
	...
	require.NoError(t, err)
	assert.Equal(t, version2b.RevID, doc.CurrentRev, "the refused write should not have changed the winning revision")
	...
	doc, err = collection.GetDocument(ctx, testDocID, db.DocUnmarshalAll)
	require.NoError(t, err)
	require.Equal(t, version2a.RevID, doc.CurrentRev, "branch a should now be the winning revision")
...
func TestSyncFnLiveBranchPromotedWithUnreadableExternalisedBody(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, version2b.RevID, doc.CurrentRev, "the refused write should not have changed the winning revision")
	assert.Equal(t, bodyKey, doc.History[version2a.RevID].BodyKey, "branch a should still reference its externalised body")
	...
	assert.Equal(t, version2b.RevID, body[db.BodyRev].(string))
	...
	body = nil
	require.NoError(t, base.JSONUnmarshal(response.Body.Bytes(), &body))
	assert.Equal(t, version2a.RevID, body[db.BodyRev].(string))
...
func setUpPromotedTombstoneBranch(t *testing.T, rt *RestTester, docID string) (winnerRev, tombstoneRev string) {
	...
	require.NoError(t, err)
	_, _, err = collection.PutExistingRevWithConflictResolution(ctx, remoteDoc, remoteHistory, true, db.NewConflictResolver(db.DefaultConflictResolver, nil), false, rawBucketDoc)
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.expectTombstoned {
				assert.Equal(t, tombstoneRev, doc.CurrentRev, "the tombstone branch should have been promoted")
				assert.True(t, doc.IsDeleted())
			} else {
				assert.Equal(t, winnerRev, doc.CurrentRev, "the refused write should not have changed the winning revision")
				assert.False(t, doc.IsDeleted())
			}
...
func TestSyncFnPromotedTombstoneWithExpiredBackupBody(t *testing.T) {
	...
	doc, err = collection.GetDocument(ctx, testDocID, db.DocUnmarshalAll)
	require.NoError(t, err)
	require.Equal(t, tombstoneRev, doc.CurrentRev, "the tombstone branch should have been promoted")
...
func setUpPromotedTombstoneWithExternalisedBody(t *testing.T, rt *RestTester, docID string) (winnerRev, tombstoneRev, bodyKey string) {
	...
	winnerRev = rt.UpdateDoc(docID, version1a, fmt.Sprintf(`{"channels":["chanA"],"gen":2,"padding":%q}`, padding)).RevID

	collection, ctx := rt.GetSingleTestDatabaseCollection()
	doc, err := collection.GetDocument(ctx, docID, db.DocUnmarshalAll)
	require.NoError(t, err)
	require.Equal(t, winnerRev, doc.CurrentRev, "the live revision should be the winning revision")
