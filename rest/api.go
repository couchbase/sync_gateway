	...
	if params.File != "" {
		...
		if profileName != "" {
			...
			if profile := pprof.Lookup(profileName); profile != nil {
				profile.WriteTo(f, 0)
				base.LogfR("Wrote %s profile to %s", base.MD(profileName), base.MD(params.File))
			} else {
				return base.HTTPErrorf(http.StatusNotFound, "No such profile %q", profileName)
			}
		} else {
			base.LogfR("Starting CPU profile to %s ...", base.MD(params.File))
...
func (h *handler) handleHeapProfiling() error {
	...
	base.LogfR("Dumping heap profile to %s ...", base.MD(params.File))
	f, err := os.Create(params.File)
	if err != nil {
		return err
	}
