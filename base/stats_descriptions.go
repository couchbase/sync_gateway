...
const (
	...
	NumEntriesInLateFeedDesc = "The total number of late-arriving-sequence entries currently held across all channels' late-arriving-sequence queues (lateLogs), used to let continuous _changes feeds catch up on skipped sequences that resolved out of order. Each queue always retains one front entry as listener-tracking infrastructure (the seq-0 sentinel at first, then whichever entry survives as the tail); that single retained placeholder is excluded per channel (the gauge counts len(lateLogs)-1), so it reflects the actual late backlog rather than having a floor equal to the number of live channel caches. Grows as skipped sequences resolve late while a listener is registered on a channel, and shrinks when entries are purged because all listeners advanced/released them, because the length/age caps prune them, or because a channel cache is evicted/cleared."
