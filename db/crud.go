...
func (c *DatabaseCollection) getRevision(ctx context.Context, doc *Document, revid string) (bodyBytes []byte, attachments AttachmentsMeta, set base.Set, err error) {
	...
	if bodyBytes == nil {
		...
		bodyBytes, chanBytes, err = c.getOldRevisionJSON(ctx, doc.ID, revid)
	...
	if len(chanBytes) > 0 {
		var backupRevChans BackupRevisionChannels
		err = base.JSONUnmarshal(chanBytes, &backupRevChans)
		if err != nil {
			return nil, nil, nil, err
		}
		channelSet = backupRevChans.Channels.CurrentChannels()
	}
	...
	return bodyBytes, attachments, channelSet, nil
}
...
func (db *DatabaseCollectionWithUser) backupAncestorRevs(ctx context.Context, doc *Document, newDoc *Document) {
	...
	db.backupRevisionJSON(ctx, doc.ID, ancestorRevId, json, BackupRevisionChannels{
		Channels: doc.Channels,
	})
...
func (db *DatabaseCollectionWithUser) tombstoneActiveRevision(ctx context.Context, doc *Document, revID string) (tombstoneRevID string, err error) {
	...
	if err == nil {
		_ = db.setOldRevisionJSONBody(ctx, doc.ID, revID, bodyBytes, db.oldRevExpirySeconds(), BackupRevisionChannels{
			Channels: doc.Channels,
		})
	}
	doc.RemoveBody()
...
func (db *DatabaseCollectionWithUser) postWriteUpdateHLV(ctx context.Context, doc *Document, casOut uint64) *Document {
	...
	if db.UseXattrs() && backupRev {
		...
		_ = db.setOldRevisionJSONBody(ctx, doc.ID, revHash, newBodyWithAtts, db.deltaSyncRevMaxAgeSeconds(), BackupRevisionChannels{
			Channels: doc.Channels,
		})
