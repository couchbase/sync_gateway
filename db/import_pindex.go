...
func (il *importListener) NewImportDest(janitorRollback func()) (cbgt.Dest, error) {
	...
	importDest, _, err := base.NewDCPDest(il.loggingCtx, callback, il.bucket, maxVbNo, true, importFeedStatsMap.Map, base.DCPImportFeedID, importPartitionStat, &il.activeVBucketCount, il.checkpointPrefix, il.metadataKeys, janitorRollback)
	if err != nil {
		return nil, err
	}
