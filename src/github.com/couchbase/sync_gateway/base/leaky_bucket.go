...
func (b *LeakyBucket) StartTapFeed(args walrus.TapArguments) (walrus.TapFeed, error) {
	...
	if b.config.TapFeedDeDuplication {
		...
		callback := func(event *walrus.TapEvent) bool {
...
type EventUpdateFunc func(event *walrus.TapEvent) bool

func (b *LeakyBucket) wrapFeed(args walrus.TapArguments, callback EventUpdateFunc) (walrus.TapFeed, error) {
...
func (b *LeakyBucket) wrapFeedForDeduplication(args walrus.TapArguments) (walrus.TapFeed, error) {
	// create an output channel
...
// An implementation of a walrus tap feed that wraps
...
func (feed *wrappedTapFeedImpl) Events() <-chan walrus.TapEvent {
	return feed.channel
}
