...
func TestStartupConfigMerge(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			config:   StartupConfig{},
			override: StartupConfig{OptReplicator: &ReplicatorConfig{MaxHeartbeat: base.NewConfigDuration(time.Second * 5)}},
			expected: StartupConfig{OptReplicator: &ReplicatorConfig{MaxHeartbeat: base.NewConfigDuration(time.Second * 5)}},
		},
	}
