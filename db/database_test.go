...
func TestGetRemovedAsUser(t *testing.T) {
	...
	cacheHitCounter, cacheMissCounter, memoryCacheStat := db.DatabaseContext.DbStats.Cache().RevisionCacheHits, db.DatabaseContext.DbStats.Cache().RevisionCacheMisses, db.DatabaseContext.DbStats.Cache().RevisionCacheTotalMemory
	collection.dbCtx.revisionCache = NewShardedLRURevisionCache(DefaultRevisionCacheShardCount, DefaultRevisionCacheSize, 0, backingStoreMap, cacheHitCounter, cacheMissCounter, memoryCacheStat)
...
func TestGetRemovedAndDeleted(t *testing.T) {
	...
	// Manually flush the rev cache
	// After expiry from the rev cache and removal of doc backup, try again
	cacheHitCounter, cacheMissCounter, memoryCacheStats := db.DatabaseContext.DbStats.Cache().RevisionCacheHits, db.DatabaseContext.DbStats.Cache().RevisionCacheMisses, db.DatabaseContext.DbStats.Cache().RevisionCacheTotalMemory
	collection.dbCtx.revisionCache = NewShardedLRURevisionCache(DefaultRevisionCacheShardCount, DefaultRevisionCacheSize, 0, backingStoreMap, cacheHitCounter, cacheMissCounter, memoryCacheStats)
	err = collection.PurgeOldRevisionJSON(ctx, "doc1", rev2id)
	assert.NoError(t, err, "Purge old revision JSON")
