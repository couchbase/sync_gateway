...
//	defer btc.Close()
//
//	//btc2 := btcRunner.NewBlipTesterClientOptsWithRT(rt, nil)
//	//defer btc2.Close()
//
//	err = btc.StartPull()
//	assert.NoError(t, err)
//
//	//err = btcRunner.StartPull(btc2.id)
//	//assert.NoError(t, err)
//
//	// Create doc revision with attachment on SG.
//	bodyText := `{"greetings":[{"hi": "alice"}],"_attachments":{"hello.txt":{"data":"aGVsbG8gd29ybGQ="}}}`
...
//
//	data, ok := btc.WaitForRev(docID, version.Rev)
//	assert.True(t, ok)
//	bodyTextExpected := `{"greetings":[{"hi":"alice"}],"_attachments":{"hello.txt":{"revpos":1,"length":11,"stub":true,"digest":"sha1-Kq5sNclPz7QV2+lfQIuc6R7oRu0="}}}`
//	require.JSONEq(t, bodyTextExpected, string(data))
//
...
//		subChangesRequest.SetProfile(db.MessageProposeChanges)
//		subChangesRequest.SetBody([]byte("[]"))
//		err := btc.pushReplication.sendMsg(subChangesRequest)
//		assert.NoError(t, err)
//		proposeChangesResponse := subChangesRequest.Response()
...
//	// Update the replicated doc at client along with keeping the same attachment stub.
//	bodyText = `{"greetings":[{"hi":"bob"}],"_attachments":{"hello.txt":{"revpos":1,"length":11,"stub":true,"digest":"sha1-Kq5sNclPz7QV2+lfQIuc6R7oRu0="}}}`
//	rev, err := btc.PushRev(docID, version.Rev, []byte(bodyText))
//	require.NoError(t, err)
//
//	// Wait for the document to be replicated at SG
//	_, ok = btc.pushReplication.WaitForMessage(2)
//	assert.True(t, ok)
//
//	resp := rt.SendAdminRequest(http.MethodGet, "/{{.keyspace}}/"+docID+"?rev="+rev, "")
//	assert.Equal(t, http.StatusOK, resp.Code)
//	var respBody db.Body
//	assert.NoError(t, base.JSONUnmarshal(resp.Body.Bytes(), &respBody))
//
//	bodyText = `{"greetings":[{"hi":"bob"}],"_attachments":{"hello111.txt":{"revpos":1,"length":11,"stub":true,"digest":"sha1-Kq5sNclPz7QV2+lfQIuc6R7oRu0="}}}`
//	rev, err = btc.PushRev(docID, rev, []byte(bodyText))
//	require.NoError(t, err)
//
//	assert.Equal(t, docID, respBody[db.BodyId])
//	greetings := respBody["greetings"].([]interface{})
...
//	assert.True(t, hello["stub"].(bool))
//
//	bodyText = `{"greetings":[{"hi": "alice"}],"_attachments":{"hello1.txt":{"data":"aGVsbG8gd29ybGQ="}}}`
//	version = btc.rt.PutDoc("doc10", bodyText)
//	data, ok = btc.WaitForRev("doc10", version.Rev)
//	assert.True(t, ok)
//
//	bodyText = `{"greetings":[{"hi": "alice"}],"_attachments":{"hello11.txt":{"data":"aGVsbG8gd29ybGQ="}}}`
//	version = btc.rt.PutDoc("doc11", bodyText)
//	data, ok = btc.WaitForRev("doc11", version.Rev)
//	assert.True(t, ok)
//
//	time.Sleep(5 * time.Second)
//
//	f, err := os.Create("/Users/gregorynewman-smith/Documents/Mobile/3.1-sync_gateway/sync_gateway/db/prof.out")
//	require.NoError(t, err)
//	err = pprof.WriteHeapProfile(f)
//	require.NoError(t, err)
//
//	fileCloseError := f.Close()
//	require.NoError(t, fileCloseError)
//
//}
