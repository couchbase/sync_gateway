...
func TestLegacyConfigToStartupConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			base:     StartupConfig{OptUnsupported: &UnsupportedConfig{StatsLogFrequency: base.NewConfigDuration(time.Minute)}},
			input:    LegacyServerConfig{Unsupported: &UnsupportedServerConfigLegacy{StatsLogFrequencySecs: base.UintPtr(10)}},
			expected: StartupConfig{OptUnsupported: &UnsupportedConfig{StatsLogFrequency: base.NewConfigDuration(time.Second * 10)}},
		},
		{
