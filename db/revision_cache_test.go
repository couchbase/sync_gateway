...
func TestInvalidRevTreeRevisionCacheHandling(t *testing.T) {
	...
	revisionCache.Put(ctx, DocumentRevision{
		DocID:      docID,
		RevID:      "2-def",
	...
	seeded, found := revisionCache.Peek(ctx, docID, "2-def")
	require.True(t, found, "failed to seed the cache for cv %q", preRepairCV)
	...
	t.Run("stale pre-repair CV entry is evicted by the repair", func(t *testing.T) {
		...
		assert.False(t, found, "the pre-repair entry should have been removed by the repair")
	})
	...
	t.Run("a CV load repopulates the CV key with the repaired revision", func(t *testing.T) {
		docRev, err := revisionCache.GetWithCV(ctx, docID, preRepairVersion, RevCacheOmitDelta, false)
		require.NoError(t, err)
...
func TestInvalidRevTreeCVEntryCachedByFailedRepair(t *testing.T) {
	...
	cached, found := collection.revisionCache.Peek(ctx, docID, "2-def")
	require.True(t, found, "precondition: the failed repair should have left an entry behind")
	...
	_, found = collection.revisionCache.Peek(ctx, docID, "2-def")
	assert.False(t, found, "the repair should have evicted the pre-repair entry")
