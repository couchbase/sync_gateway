                        ...
                        stage('CE') {
                            ...
                            steps{
                                ...
                                withEnv(["PATH+GO=${env.GOTOOLS}/bin", "TRAVIS_BRANCH=${env.BRANCH}", "TRAVIS_PULL_REQUEST=${env.CHANGE_ID}", "TRAVIS_JOB_ID=${env.BUILD_NUMBER}"]) {
                                    ...
                                    sh '2>&1 go test -shuffle=on -timeout=20m -coverpkg=./... -coverprofile=cover_ce.out -race -count=1 -v ./rest -run TestCBLRevposHandling > verbose_ce.out.raw || true'
                        ...
                        stage('EE') {
                            steps {
                                withEnv(["PATH+GO=${env.GOTOOLS}/bin"]) {
                                    ...
                                    sh "2>&1 go test -shuffle=on -timeout=20m -tags ${EE_BUILD_TAG} -coverpkg=./... -coverprofile=cover_ee.out -race -count=1 -v ./rest -run TestCBLRevposHandling > verbose_ee.out.raw || true"
