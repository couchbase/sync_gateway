...
func TestBlipMessageAfterUserDelete(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		unsubChangesRequest.SetProfile(db.MessageUnsubChanges)
		client.addCollectionProperty(unsubChangesRequest)
		client.pullReplication.sendMsg(unsubChangesRequest)
...
func TestBlipPullReplicationUserDeleted(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		client := btcRunner.NewBlipTesterClientOptsWithRT(rt, &BlipTesterClientOpts{Username: username, SupportedBLIPProtocols: SupportedBLIPProtocols})
		defer client.Close()
		btcRunner.StartPull(client.id)
...
func TestMissingBackupBodyWhenTombstoneBranchPromotedToWinningBranch(t *testing.T) {
	// The promoted tombstone is only reached here by an on-demand import of a delete made behind
	// Sync Gateway's back, which requires xattrs.
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		for _, testCase := range testCases {
			t.Run(testCase.name, func(t *testing.T) {
				...
				localGen, _ := db.ParseRevID(ctx, version.RevID)
				...
				_, _, err := collection.PutExistingRevWithConflictResolution(ctx, remoteDoc, remoteHistory,
					true, db.NewConflictResolver(db.DefaultConflictResolver, nil), false, rawBucketDoc)
				...
				require.NoError(t, err)
				require.Equal(t, tombstoneLeaf, importedDoc.CurrentRev, "the tombstone branch should have been promoted")
				...
				tombstoneVersion := DocVersion{RevID: tombstoneLeaf}
				...
				//
				// 3.3.8's BlipTesterCollectionClient cannot hand back the norev message it received, so
				// the norev is asserted through the stat rather than by inspecting the message.
				base.RequireWaitForStat(t, rt.GetDatabase().DbStats.CBLReplicationPull().NoRevSendCount.Value, 1)
