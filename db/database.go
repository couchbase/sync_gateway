...
type DatabaseContext struct {
	...
	ResyncManager               ResyncBackgroundManager
	TombstoneCompactionManager  *Compactor
	AttachmentCompactionManager *Compactor
	...
	CompactState                uint32                   // State of database compaction
...
func NewDatabaseContext(dbName string, bucket base.Bucket, autoImport bool, options DatabaseContextOptions) (dbc *DatabaseContext, returnedError error) {
	...
	dbContext.TombstoneCompactionManager = NewTombstoneCompactionBackgroundManager()
	dbContext.AttachmentCompactionManager = NewAttachmentCompactionBackgroundManager()
...
func (db *Database) Compact(skipRunningStateCheck bool, tombstoneCompactionManager *TombstoneCompactionBackgroundManager) (int, error) {
	...
	defer func() {
		if tombstoneCompactionManager != nil {
			tombstoneCompactionManager.SetTombstonesCompacted(purgedDocCount)
		}
	}()
	...
	for {
		...
		for results.Next(&tombstonesRow) {
			if db.TombstoneCompactionManager.ShouldStop() {
				closeErr := results.Close()
				if closeErr != nil {
					return 0, closeErr
				}
