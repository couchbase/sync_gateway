...
func legacyServerMain(ctx context.Context, osArgs []string, flagStartupConfig *StartupConfig) error {
	...
	svrctx, err := setupServerContext(&sc, false)
	if err != nil {
		return err
	}
	ctx = base.LogContextWith(ctx, &base.ServerLogContext{
		ConfigGroupID: svrctx.config.Bootstrap.ConfigGroupID,
	})
	svrctx.initialStartupConfig = initialStartupConfig

	err = svrctx.CreateLocalDatabase(databases)
	if err != nil {
		return err
	}
