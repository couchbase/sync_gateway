...
// but returning 0 if missing or unparseable.  If value is missing isSet will return false.
// If allowZero is true, values coming in as zero will stay zero, instead of being set to the minValue.
func getRestrictedIntQuery(values url.Values, query string, defaultValue, minValue, maxValue uint64, allowZero bool) (value uint64, isSetRet bool) {
	isSet := true
