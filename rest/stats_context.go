...
func newCpuStatsSnapshot() (snapshot *cpuStatsSnapshot, err error) {
	...
	process, err := process2.NewProcess(int32(pid))
	if err != nil {
		return nil, err
	}
	snapshot.process = process

	cpuTimes, err := process.Times()
	if err != nil {
		return nil, err
	}
	snapshot.cpuTimes = cpuTimes

	snapshot.cpuTime = time.Now()
	return snapshot, nil
}
...
func (statsContext *statsContext) calculateProcessCpuPercentage() (cpuPercentUtilization float64, err error) {
	...
	prevCpuTimes := statsContext.cpuStatsSnapshot.cpuTimes
	prevCpuTime := statsContext.cpuStatsSnapshot.cpuTime

	timeDelta := currentSnapshot.cpuTime.Sub(prevCpuTime).Seconds() * float64(numCpu)
	procDelta := currentSnapshot.cpuTimes.Total() - prevCpuTimes.Total()

	cpuPercentUtilization = ((procDelta / timeDelta) * 100) * float64(numCpu)
	statsContext.cpuStatsSnapshot = currentSnapshot
