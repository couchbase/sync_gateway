...
func TestDocAttachmentOnRemovedRev(t *testing.T) {
	...
	//Create a test user
	user, err = a.NewUser("user1", "letmein", channels.SetOfOrPanic("foo"))
	a.Save(user)
...
func TestLogin(t *testing.T) {
	...
	user, err = a.NewUser("pupshaw", "letmein", channels.SetOfOrPanic("*"))
	a.Save(user)
...
func TestAllDocsAccessControl(t *testing.T) {
	...
	// Create a user:
	alice, err := a.NewUser("alice", "letmein", channels.SetOfOrPanic("Cinemax"))
	a.Save(alice)
...
func TestChannelAccessChanges(t *testing.T) {
	...
	// Create users:
	alice, err := a.NewUser("alice", "letmein", channels.SetOfOrPanic("zero"))
	a.Save(alice)
	zegpold, err := a.NewUser("zegpold", "letmein", channels.SetOfOrPanic("zero"))
	a.Save(zegpold)
...
func TestAccessOnTombstone(t *testing.T) {
	...
	// Create user:
	bernard, err := a.NewUser("bernard", "letmein", channels.SetOfOrPanic("zero"))
	a.Save(bernard)
...
func TestRoleAccessChanges(t *testing.T) {
	...
	// Create users:
	alice, err := a.NewUser("alice", "letmein", channels.SetOfOrPanic("alpha"))
	a.Save(alice)
	zegpold, err := a.NewUser("zegpold", "letmein", channels.SetOfOrPanic("beta"))
	a.Save(zegpold)

	hipster, err := a.NewRole("hipster", channels.SetOfOrPanic("gamma"))
	a.Save(hipster)
...
func TestStarAccess(t *testing.T) {
	...
	//
	bernard, err := a.NewUser("bernard", "letmein", channels.SetOfOrPanic("books"))
	a.Save(bernard)
	...
	// Create a user:
	fran, err := a.NewUser("fran", "letmein", channels.SetOfOrPanic("*"))
	a.Save(fran)
...
func TestDocIDFilterResurrection(t *testing.T) {
	...
	//Create User
	a := rt.ServerContext().Database("db").Authenticator()
	jacques, err := a.NewUser("jacques", "letmein", channels.SetOfOrPanic("A", "B"))
	assert.NoError(t, err)
	a.Save(jacques)
...
func TestNumAccessErrors(t *testing.T) {
	...
	//Create a test user
	user, err := a.NewUser("user", "letmein", channels.SetOfOrPanic("A"))
	assert.NoError(t, err)
	a.Save(user)
