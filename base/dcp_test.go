...
func TestCBGTIndexCreation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg := cbgt.NewCfgMem()
			context, err := initCBGTManager(ctx, bucket, spec, "", nil, cfg, "testIndexCreation", tc.dbName)
...
func TestCBGTIndexCreationSafeLegacyName(t *testing.T) {
	...
	ctx := TestCtx(t)
	cfg := cbgt.NewCfgMem()
	context, err := initCBGTManager(ctx, bucket, spec, "", nil, cfg, "testIndexCreation", testDbName)
	assert.NoError(t, err)
...
func TestCBGTIndexCreationUnsafeLegacyName(t *testing.T) {
	...
	ctx := TestCtx(t)
	cfg := cbgt.NewCfgMem()
	context, err := initCBGTManager(ctx, bucket, spec, "", nil, cfg, "testIndexCreation", unsafeTestDBName)
	assert.NoError(t, err)
...
func TestConcurrentCBGTIndexCreation(t *testing.T) {
	...
	for i := 0; i < managerCount; i++ {
		go func(i int, terminatorChan chan struct{}) {
			...
			context, err := initCBGTManager(ctx, bucket, spec, "", nil, cfg, managerUUID, testDBName)
			assert.NoError(t, err)
