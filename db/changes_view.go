...
func (dbc *DatabaseContext) getChangesInChannelFromView(
	...
	base.LogToR("Cache", "  Querying 'channels' view for %q (start=#%d, end=#%d, limit=%d)", base.UD(channelName), startSeq, endSeq, options.Limit)
	...
	if elapsed := time.Since(start); elapsed > 200*time.Millisecond {
		base.LogfR("changes_view: Query took %v to return %d rows.  Channel: %s StartSeq: %d EndSeq: %d Limit: %d",
