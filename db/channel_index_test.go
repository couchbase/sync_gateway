...
func NewChannelIndex(vbNum int, sequenceGap int, name string, tester testing.TB) *channelIndexTest {
...
func BenchmarkChannelIndexSimpleGet(b *testing.B) {
	...
	// num vbuckets
	vbCount := 1024
	index := NewChannelIndex(vbCount, 0, "basicChannel", b)
...
func MultiChannelIndexSimpleGet(b *testing.B, numChannels int) {
	...
	testIndexBucket := base.GetTestIndexBucket(b)
...
func MultiChannelIndexBulkGet(b *testing.B, numChannels int) {
	...
	testBucket := base.GetTestIndexBucket(b)
...
func TestChannelIndexBulkGet10(t *testing.T) {
	...
	vbCount := 1024
	numChannels := 10
	testBucket := base.GetTestIndexBucket(t)
	defer testBucket.Close()
...
func TestChannelIndexSimpleReadSingle(t *testing.T) {
	...
	// num vbuckets
	vbCount := 1024
	index := NewChannelIndex(vbCount, 10, "basicChannel", t)
	defer index.indexBucket.Close()
...
func TestChannelIndexSimpleReadBulk(t *testing.T) {
	...
	// num vbuckets
	vbCount := 1024
	index := NewChannelIndex(vbCount, 0, "basicChannel", t)
	defer index.indexBucket.Close()
