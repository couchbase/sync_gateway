...
func (s *sequenceHasher) GetClock(sequence string) (*base.SequenceClockImpl, error) {
	...
	if uint16(len(storedClocks.Sequences)) <= seqHash.collisionIndex {
		base.Warnf(base.KeyAll, "Stored hash not found for sequence [%s] collision index [%d], #storedClocks:%d, returning zero clock", sequence, seqHash.collisionIndex, len(storedClocks.Sequences))
	} else {
...
func (s *sequenceHasher) loadClocks(hashValue uint64) (*storedClocks, error) {
	...
	if err = stored.Unmarshal(bytes); err != nil {
		base.Warnf(base.KeyAll, "Error unmarshalling stored clocks for key [%s], returning zero sequence", key)
