...
class TaskRunner(object):
    ...
    def zip(self, filename, node):
        ...
        prefix = "sgcollect_info_%s_%s" % (node, self.start_time)
...
def make_product_task(guts, initargs_path, options):
    ...
    _tasks = [
        ...
        [AllOsTask("couchbase logs (%s)" % name, "sgbrowse_logs %s" % name,
        ...
        [AllOsTask("memcached stats %s" % kind,
                   ...
                   flatten(["sgstats", "-a", "127.0.0.1:%s" % read_guts(guts, "memcached_port"), kind, "-b", read_guts(guts, "memcached_admin"), "-p", read_guts(guts, "memcached_pass")]),
                   log_file="stats.log",
                   timeout=60)
    ...
    _tasks = flatten([lookup_tasks, query_tasks, index_tasks, _tasks])
...
of utilites used by sgcollect_info'''
...
def generate_upload_url(parser, options, zip_filename):
    ...
    if options.upload_host:
        ...
        if options.upload_ticket:
            upload_url = "https://%s/%s/%d/%s" % (upload_host, customer, options.upload_ticket, fname)
        else:
...
class SgcollectInfoOptions(optparse.Option):
...
def main():
    ...
    parser = optparse.OptionParser(usage=USAGE, option_class=SgcollectInfoOptions)
    ...
    if os.name == 'posix':
      ...
      path = [mydir, 'C:\Program Files (x86)\Couchbase', os.environ['PATH']]
      os.environ['PATH'] = ';'.join(path)
  # elif os.name == 'darwin'
    # TODO: Support OSX?!
    ...
    if platform.system() == 'Windows':
        prefix = 'C:\Program Files (x86)\Couchbase'
        runbase = prefix
    elif platform.system() == 'Darwin':
        prefix = '/Applications/Sync Gateway/Contents/ something /bin' # TODO: grab this from the app bundle.
        runbase = prefix
    else:
        prefix = '/opt/couchbase-sync-gateway' # TODO: This varies by distro?
        runbase = '/home/sync_gateway'

    guts = {"config_path" : os.path.join(runbase, "sync_gateway.json"),
            "log_path" : os.path.join(runbase, "logs")}

    guts_initargs_path = prefix

    zip_node = read_guts(guts, "node") # TODO: not sure what 'node' should map to here. Need to run the regular cbcollect_info under the debugger.
    runner.run(
