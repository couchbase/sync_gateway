...
// isStopping returns true once Stop has begun tearing the manager down.  clusterSubscribeTerminator is
// closed as the first action in Stop, before activeReplicatorsLock is acquired, so a caller that checks
// this while holding that lock either observes the manager as stopping, or is guaranteed to complete
// before Stop's teardown loop runs.
...
func (m *sgReplicateManager) Stop() {
	...
	// Stop the heartbeat listener before removing the local node below.  RemoveNode writes the cluster
	// config, and the listener re-registers the local node on any cfg event that finds it absent -
	// including the event generated by its own removal.  Re-registering rebalances replications back onto
	// the node that is being torn down, and the resulting cfg write is itself another cfg event.
	...
	for replicationID, repl := range m.activeReplicators {
		err := repl.Stop()
		if err != nil {
...
func (m *sgReplicateManager) RefreshReplicationCfg(ctx context.Context) error {
	...
	// Once Stop has begun, this refresh must not (re)start anything.  SubscribeCfgChanges' goroutine
	// selects on both the cfg event channel and clusterSubscribeTerminator, so a cfg change arriving as
	// the terminator is closed can be serviced instead of the exit.  Without this check that refresh can
	// run after Stop's teardown loop has already walked activeReplicators - via any of the three paths
	// below - leaving a running replicator that nothing will ever stop.
	if m.isStopping() {
		base.DebugfCtx(m.loggingCtx, base.KeyCluster, "Manager is stopping - skipping replication cfg refresh")
		return nil
	}
	...
	go func() {
		...
		for {
			...
			case <-cfgEvents:
				...
				// Don't re-register a node whose manager is being torn down - that would rebalance
				// replications back onto it.  Stop closes this listener before removing the local node,
				// so this only covers an event already in flight at that point.
				if !localNodeRegistered && !l.mgr.isStopping() {
