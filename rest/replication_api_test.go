...
func TestReplicateDocIDNullCharPrefix(t *testing.T) {
	...
	docID := "\u0000rt2doc1"
	// Doc needs to be inserted into the bucket manually, because null bytes are invalid in URLs
	require.NoError(t, tb2.Set(docID, 0, nil, map[string]any{"channels": []string{"alice"}}))
	require.NoError(t, tb2.Set("before", 0, nil, map[string]any{"channels": []string{"alice"}}))
	require.NoError(t, rt2.WaitForPendingChanges())

	// Verify it got imported correctly
	baseDoc, err := rt2.GetDatabase().GetDocument(base.TestCtx(t), docID, db.DocUnmarshalAll)
	require.NoError(t, err)

	baseDoc, err = rt2.GetDatabase().GetDocument(base.TestCtx(t), "before", db.DocUnmarshalAll)
	require.NoError(t, err)
	...
	ar := db.NewActiveReplicator(rt1.Context(), &db.ActiveReplicatorConfig{
		ID:          t.Name(),
		Direction:   db.ActiveReplicatorTypePull,
	...
	assert.Equal(t, "before", changesResults.Results[0].ID)

	repDoc, err := rt1.GetDatabase().GetDocument(base.TestCtx(t), docID, db.DocUnmarshalAll)
	assert.Error(t, err)

	repDoc, err = rt1.GetDatabase().GetDocument(base.TestCtx(t), "before", db.DocUnmarshalAll)
	require.NoError(t, err)
	require.Equal(t, baseDoc.RevID, repDoc.RevID)

	// Confirm replication is still continuing
	require.NoError(t, tb2.Set("after", 0, nil, map[string]any{"channels": []string{"alice"}}))
	require.NoError(t, rt2.WaitForPendingChanges())
	baseDoc, err = rt2.GetDatabase().GetDocument(base.TestCtx(t), "after", db.DocUnmarshalAll)
	require.NoError(t, err)
	...
	// Make sure changes have arrived over the changes feed
	changesResults, err = rt1.WaitForChanges(1, "/db/_changes?since="+fmt.Sprintf("%s", changesResults.Last_Seq), "", true)
	require.NoError(t, err)
	require.Len(t, changesResults.Results, 1)
	assert.Equal(t, "after", changesResults.Results[0].ID)
