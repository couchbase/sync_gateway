...
//   - Client pulls with delta sync enabled; handleChanges reports rev1RevTreeID as the known rev
...
//
//	history property: [rev2RevTreeID, rev1RevTreeID] (no HLV pv since doc has only been updated by one HLV-aware peer)
...
func TestDeltaSyncSendHistoryWithLegacyClient(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T) {
		...
		btcRunner.AddRevTreeRev(client.id, "doc1", rev1ID, EmptyDocVersion(), []byte(`{"key": "val1"}`))

		btcRunner.StartOneshotPull(client.id)
		msg := btcRunner.WaitForPullRevMessage(client.id, "doc1", docVersion2)
...
func TestDeltaSyncSendHistoryWithLegacyClientAndHLVHistory(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T) {
		...
		assert.Equal(t, newDoc.HLV.GetCurrentVersionString(), msg.Properties[db.RevMessageRev])
