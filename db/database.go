...
type DatabaseContext struct {
	...
	backgroundTasks    []BackgroundTask         // List of terminated background goroutines
...
func (ctx *DatabaseContext) waitForBGTCompletion(waitTimeMax time.Duration, tasks []BackgroundTask) {
	...
	for i, t := range tasks {
		...
		case <-time.After(waitTime):
			for _, v := range tasks[i:] {
				base.InfofCtx(context.Background(), base.KeyAll, "Timeout after %v of waiting for "+
					"background task %q to terminate", waitTimeMax, v.name)
			}
			return
		}
	}
