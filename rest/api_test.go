...
func TestPublicRESTBytesReadStat(t *testing.T) {
	...
	publicBytesreadStat := rt.GetDatabase().DbStats.DatabaseStats.PublicRestBytesRead.Value()
	...
	publicBytesreadStat1 := rt.GetDatabase().DbStats.DatabaseStats.PublicRestBytesRead.Value()
	require.Greater(t, publicBytesreadStat1, publicBytesreadStat)
	...
	// send another public request to check stat increments, but check it increments by correct bytes value
	bodyString := fmt.Sprint(`{"foo":"bar", "channels":["ABC"]}`)
	byteArray := []byte(bodyString)

	resp = rt.SendUserRequest(http.MethodPut, "/{{.keyspace}}/doc2", bodyString, "greg")
	RequireStatus(t, resp, http.StatusCreated)

	publicBytesreadStat1 += int64(len(byteArray))
	_, ok = base.WaitForStat(func() int64 {
		return rt.GetDatabase().DbStats.DatabaseStats.PublicRestBytesRead.Value()
	}, publicBytesreadStat1)
	require.True(t, ok)
