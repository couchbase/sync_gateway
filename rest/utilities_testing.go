...
// failing the test if the feed is still running.
func (f *ContinuousChangesFeed) RequireEnded() []db.ChangeEntry {
	t := f.rt.TB()
	t.Helper()
	select {
	case <-f.done:
	case <-time.After(30 * time.Second):
		require.Fail(t, "continuous changes feed still running")
	}
	changes, err := f.rt.ReadContinuousChanges(f.response)
	require.NoError(t, err)
	return changes
}
