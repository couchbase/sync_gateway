...
func HasCachingFeedDelay(t testing.TB) bool {
	t.Helper()
	return totalCachingFeedDelay(t) > 0
}
...
func GetCachingFeedDelayFactor(t testing.TB) time.Duration {
	t.Helper()
	cachingDelay := totalCachingFeedDelay(t)
	if cachingDelay == 0 {
		return 1
	}
...
// totalCachingFeedDelay returns the worst-case artificial delay configured for a single caching feed event. A
// principal doc sleeps for both delays, so they are summed. Wait helpers that shorten their timeout when the feed is
// undelayed must see a principal doc delay too, or they time out against it.
func totalCachingFeedDelay(t testing.TB) time.Duration {
	t.Helper()
...
func (db *Database) SetUserForTest(_ testing.TB, user auth.User) {
	db.user = user
}
...
func NewSingleChannelCacheForTest(_ testing.TB, queryHandler ChannelQueryHandler, channel channels.ID, validFrom uint64, cacheStats *base.CacheStats) *singleChannelCacheImpl {
...
func GetSingleChannelCacheForTest(_ testing.TB, ctx context.Context, cache ChannelCache, ch channels.ID) (SingleChannelCache, error) {
...
// subscribes to.
func CachingFeedCollectionsForTest(_ testing.TB, metadataStore base.DataStore, scopes map[string]Scope) base.CollectionNameSet {
