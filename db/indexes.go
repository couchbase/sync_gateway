...
var (
	...
	indexExpressions = map[SGIndexType]string{
		...
		IndexChannels: "ALL (ARRAY FLATTEN_KEYS(op.name, LEAST($sync.sequence,op.val.seq), IFMISSING(op.val.rev,null), IFMISSING(op.val.del,null)) FOR op IN OBJECT_PAIRS($sync.channels) END), " +
			"$sync.rev, $sync.sequence, $sync.flags",
	...
	readinessQueries = map[SGIndexType]string{
		...
		// 	"LIMIT 1",
		IndexChannels: "SELECT  LEAST($sync.sequence, op.val.seq) AS sequence " +
			"FROM `%s` " +
			"USE INDEX ($idx) " +
			"UNNEST OBJECT_PAIRS($sync.channels) AS op " +
			"WHERE op.name = 'foo' AND LEAST($sync.sequence, op.val.seq) BETWEEN  0 AND 1 " +
			"ORDER BY op.name, LEAST($sync.sequence, op.val.seq),IFMISSING(op.val.rev,null),IFMISSING(op.val.del,null) " +
			"LIMIT 1",
	}
)
