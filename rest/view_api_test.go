...
func TestViewQueryUserAccess(t *testing.T) {
	...
	password := "123456"
	testUser, _ := a.NewUser("testUser", password, channels.SetOfOrPanic("*"))
	a.Save(testUser)
...
func TestUserViewQuery(t *testing.T) {
	...
	// Create a user:
	password := "123456"
	quinn, _ := a.NewUser("quinn", password, channels.SetOfOrPanic("Q", "q"))
	a.Save(quinn)
...
func TestViewQueryWrappers(t *testing.T) {
	...
	a := rt.ServerContext().Database("db").Authenticator()
	testUser, err := a.NewUser("testUser", "password", channels.SetOfOrPanic("userchannel"))
	assert.NoError(t, err)
	err = a.Save(testUser)
	assert.NoError(t, err)
