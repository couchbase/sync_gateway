...
func TestManyChannelsRemovedOnDocUpdate(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T) {
		...
		if btc.UseHLV() {
			...
			// Version is generated by the SG HLC, not the CAS macro, so it is independent of - and must never
			// exceed - the document CAS (the constraint goxdcr enforces: cv.ver <= cas).
