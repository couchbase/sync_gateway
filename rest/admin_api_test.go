...
func TestUserAPI(t *testing.T) {
	...
	log.Printf("GET the user and make sure the result is OK")
	...
	log.Printf("Check the list of all users")
	...
	log.Printf("Check that the actual User object is correct")
	...
	log.Printf("Change the password and verify it")
	...
	log.Printf("Give the user that role")
	...
	log.Printf("GET the user and verify that it shows the channels inherited from the role")
	response = rt.SendAdminRequest("GET", "/db/_user/snej", "")
	assertStatus(t, response, 200)
	body = nil
	json.Unmarshal(response.Body.Bytes(), &body)
