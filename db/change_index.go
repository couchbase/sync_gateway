...
type ChangeIndex interface {
	...
	waitForSequenceID(sequence SequenceID, maxWaitTime time.Duration, tester testing.TB)
	...
	// Unit test support
	waitForSequence(sequence uint64, maxWaitTime time.Duration, tester testing.TB)
	waitForSequenceWithMissing(sequence uint64, maxWaitTime time.Duration, tester testing.TB)
}
