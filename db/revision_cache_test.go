...
func TestLRURevisionCacheEviction(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		docRev, err := cache.Get(ctx, docID, "1-abc", nil, RevCacheOmitBody, RevCacheOmitDelta)
	...
	for i := 0; i < 3; i++ {
		docID := strconv.Itoa(i)
		docRev, ok := cache.Peek(ctx, docID, "1-abc", nil)
		assert.False(t, ok)
	...
	for i := 0; i < 10; i++ {
		id := strconv.Itoa(i + 3)
		docRev, err := cache.Get(ctx, id, "1-abc", nil, RevCacheOmitBody, RevCacheOmitDelta)
		assert.NoError(t, err)
...
func TestLRURevisionCacheEvictionCVCache(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		docRev, err := cache.Get(ctx, docID, "1-abc", &cv, RevCacheOmitBody, RevCacheOmitDelta)
		assert.NoError(t, err)
		assert.NotNil(t, docRev.BodyBytes, "nil body for %s", docID)
		assert.Equal(t, docID, docRev.DocID)
		assert.Equal(t, int64(0), cacheMissCounter.Value())
	...
	for i := 0; i < 3; i++ {
		docID := strconv.Itoa(i)
		cv := CurrentVersionVector{VersionCAS: uint64(i), SourceID: "test"}
		docRev, ok := cache.Peek(ctx, docID, "", &cv)
		assert.False(t, ok)
		assert.Nil(t, docRev.BodyBytes)
		assert.Equal(t, int64(0), cacheMissCounter.Value()) // peek incurs no cache miss if not found
	...
	for i := 0; i < 10; i++ {
		...
		docRev, err := cache.Get(ctx, id, "1-abc", &cv, RevCacheOmitBody, RevCacheOmitDelta)
		assert.NoError(t, err)
		assert.NotNil(t, docRev.BodyBytes, "nil body for %s", id)
		assert.Equal(t, id, docRev.DocID)
		assert.Equal(t, int64(0), cacheMissCounter.Value())
...
func TestBackingStore(t *testing.T) {
	...
	docRev, err := cache.Get(base.TestCtx(t), "Jens", "1-abc", nil, RevCacheOmitBody, RevCacheOmitDelta)
	assert.NoError(t, err)
	...
	docRev, err = cache.Get(base.TestCtx(t), "Peter", "1-abc", nil, RevCacheOmitBody, RevCacheOmitDelta)
...
// - Perform a Get on a doc by cv that is not currently in the rev cache, assert we get cach miss
func TestBackingStoreCV(t *testing.T) {
	...
	// Get Rev for the first time - miss cache, but fetch the doc and revision to store
	cv := CurrentVersionVector{SourceID: "test", VersionCAS: 1}
	docRev, err := cache.Get(base.TestCtx(t), "doc1", "1-abc", &cv, RevCacheOmitBody, RevCacheOmitDelta)
	assert.NoError(t, err)
	assert.Equal(t, "doc1", docRev.DocID)
	assert.NotNil(t, docRev.Channels)
	assert.Equal(t, "test", docRev.CV.SourceID)
	assert.Equal(t, uint64(1), docRev.CV.VersionCAS)
	assert.Equal(t, int64(0), cacheHitCounter.Value())
	assert.Equal(t, int64(1), cacheMissCounter.Value())
	assert.Equal(t, int64(1), getDocumentCounter.Value())
	assert.Equal(t, int64(1), getRevisionCounter.Value())
	...
	docRev, err = cache.Get(base.TestCtx(t), "not_found", "1-abc", &cv, RevCacheOmitBody, RevCacheOmitDelta)
	assertHTTPError(t, err, 404)
	assert.Nil(t, docRev.BodyBytes)
	assert.Equal(t, int64(1), cacheHitCounter.Value())
	assert.Equal(t, int64(2), cacheMissCounter.Value())
...
func TestBypassRevisionCache(t *testing.T) {
	...
	_, ok := rc.Peek(base.TestCtx(t), key, rev1, nil)
	assert.False(t, ok)
	_, ok = rc.Peek(base.TestCtx(t), key, rev2, nil)
	assert.False(t, ok)

	// Get non-existing doc
	_, err = rc.Get(base.TestCtx(t), "invalid", rev1, nil, RevCacheOmitBody, RevCacheOmitDelta)
	assert.True(t, base.IsDocNotFoundError(err))

	// Get non-existing revision
	_, err = rc.Get(base.TestCtx(t), key, "3-abc", nil, RevCacheOmitBody, RevCacheOmitDelta)
	assertHTTPError(t, err, 404)

	// Get specific revision
	doc, err := rc.Get(base.TestCtx(t), key, rev1, nil, RevCacheOmitBody, RevCacheOmitDelta)
	assert.NoError(t, err)
...
func TestPutRevisionCacheAttachmentProperty(t *testing.T) {
	...
	docRevision, ok := collection.revisionCache.Peek(base.TestCtx(t), rev1key, rev1id, nil)
	assert.True(t, ok)
...
func TestPutExistingRevRevisionCacheAttachmentProperty(t *testing.T) {
	...
	// Get the raw document directly from the revcache, validate _attachments property isn't found
	docRevision, err := collection.revisionCache.Get(base.TestCtx(t), docKey, rev2id, nil, RevCacheOmitBody, RevCacheOmitDelta)
...
func TestRevisionImmutableDelta(t *testing.T) {
	...
	_, err := cache.Get(base.TestCtx(t), "doc1", "1-abc", nil, RevCacheOmitBody, RevCacheIncludeDelta)
	assert.NoError(t, err, "Error adding to cache")
	cache.UpdateDelta(base.TestCtx(t), "doc1", "1-abc", nil, RevisionDelta{ToRevID: "rev2", DeltaBytes: firstDelta})

	// Retrieve from cache
	retrievedRev, err := cache.Get(base.TestCtx(t), "doc1", "1-abc", nil, RevCacheOmitBody, RevCacheIncludeDelta)
	...
	cache.UpdateDelta(base.TestCtx(t), "doc1", "1-abc", nil, RevisionDelta{ToRevID: "rev3", DeltaBytes: secondDelta})
	assert.Equal(t, "rev2", retrievedRev.Delta.ToRevID)
	assert.Equal(t, firstDelta, retrievedRev.Delta.DeltaBytes)

	// Retrieve again, validate delta is correct
	updatedRev, err := cache.Get(base.TestCtx(t), "doc1", "1-abc", nil, RevCacheOmitBody, RevCacheIncludeDelta)
	assert.NoError(t, err, "Error retrieving from cache")
...
func TestSingleLoad(t *testing.T) {
	...
	_, err := cache.Get(base.TestCtx(t), "doc123", "1-abc", nil, true, false)
	assert.NoError(t, err)
}
...
func TestConcurrentLoad(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		go func() {
			_, err := cache.Get(base.TestCtx(t), "doc1", "1-abc", nil, true, false)
			assert.NoError(t, err)
			wg.Done()
		}()
	}
...
func TestRevisionCacheRemove(t *testing.T) {
	...
	docRev, err := collection.revisionCache.Get(base.TestCtx(t), "doc", rev1id, nil, true, true)
	assert.NoError(t, err)
	assert.Equal(t, rev1id, docRev.RevID)
	assert.Equal(t, int64(0), db.DbStats.Cache().RevisionCacheMisses.Value())

	collection.revisionCache.Remove("doc", rev1id, nil)
...
func TestRevCacheOperationsCV(t *testing.T) {
	...
	docRev, found := cache.Peek(base.TestCtx(t), "doc1", "1-abc", &cv)
	require.NoError(t, err)
	assert.True(t, found)
	assert.Equal(t, "doc1", docRev.DocID)
	assert.Equal(t, base.SetOf("chan1"), docRev.Channels)
	assert.Equal(t, "test", docRev.CV.SourceID)
	assert.Equal(t, uint64(1), docRev.CV.VersionCAS)
	assert.Equal(t, int64(3), cacheHitCounter.Value())
	assert.Equal(t, int64(0), cacheMissCounter.Value())

	// assert that the rev id cache has also been updated with the doc update
	docRev, found = cache.Peek(base.TestCtx(t), "doc1", "1-abc", nil)
	assert.True(t, found)
	assert.Equal(t, "doc1", docRev.DocID)
	assert.Equal(t, base.SetOf("chan1"), docRev.Channels)
	assert.Equal(t, int64(4), cacheHitCounter.Value())
	assert.Equal(t, int64(0), cacheMissCounter.Value())
	...
	cache.Remove("doc1", "1-abc", &cv)
	docRev, found = cache.Peek(base.TestCtx(t), "doc1", "1-abc", &cv)
	require.NoError(t, err)
	assert.False(t, found)
	assert.Equal(t, DocumentRevision{}, docRev)
...
func BenchmarkRevisionCacheRead(b *testing.B) {
	...
	// trigger load into cache
	for i := 0; i < 5000; i++ {
		_, _ = cache.Get(ctx, fmt.Sprintf("doc%d", i), "1-abc", nil, RevCacheOmitBody, RevCacheOmitDelta)
	}
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			...
			_, _ = cache.Get(ctx, docId, "1-abc", nil, RevCacheOmitBody, RevCacheOmitDelta)
		}
	})
}
