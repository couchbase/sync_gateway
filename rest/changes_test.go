...
func TestJumpInSequencesAtAllocatorSkippedSequenceFill(t *testing.T) {
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		assert.Equal(c, int64(1), rt.GetDatabase().DbStats.CacheStats.SkippedSequenceNodes.Value())
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		assert.Equal(c, int64(0), rt.GetDatabase().DbStats.CacheStats.SkippedSequenceNodes.Value())
	}, time.Second*10, time.Millisecond*100)
