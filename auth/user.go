...
func (user *userImpl) InitializeRoles() error {
	_, err := user.GetRoles()
	return err
}
...
func (user *userImpl) expandCollectionWildCardChannel(scope, collection string, channels base.Set) (base.Set, error) {
	if channels.Contains(ch.AllChannelWildcard) {
		allChannels, err := user.InheritedCollectionChannels(scope, collection)
		if err != nil {
			return nil, err
		}
		return allChannels.AsSet(), nil
	}
	return channels, nil
}
...
func (user *userImpl) FilterToAvailableCollectionChannels(scope, collection string, channelNames base.Set) (filtered ch.TimedSet, removed []string, err error) {
	...
	for channelName, _ := range channelNames {
		if channelName == ch.AllChannelWildcard {
			channels, err := user.InheritedCollectionChannels(scope, collection)
			if err != nil {
				return nil, nil, err
			}
			return channels, nil, nil
		}
		seq, err := user.canSeeCollectionChannelSince(scope, collection, channelName)
		if err != nil {
			return nil, nil, err
		}
...
func (user *userImpl) GetAddedChannels(channels ch.TimedSet) (base.Set, error) {
	allChannels, err := user.inheritedChannels()
	if err != nil {
		return nil, err
	}
	output := base.Set{}
	for userChannel := range allChannels {
