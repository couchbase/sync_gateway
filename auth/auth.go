...
// SequenceAllocator allocates and releases the sequences stamped onto principal documents.  Implemented
// by db.DatabaseContext, and defined here so the authenticator can allocate a sequence only once it knows
// it will write one - an allocated sequence that is never written leaves a permanent gap in the range.
type SequenceAllocator interface {
	NextSequence(ctx context.Context) (uint64, error)
	ReleaseSequence(ctx context.Context, seq uint64) error
}
...
// UpdateSequenceNumberForResync stamps the principal with a newly allocated sequence for the given resync
// run.  A no-op if the principal was already regenerated by this run, in which case no sequence is
// allocated.
func (auth *Authenticator) UpdateSequenceNumberForResync(p Principal, resyncID string, sequences SequenceAllocator) error {
	...
	seq, err := sequences.NextSequence(auth.LogCtx)
	if err != nil {
		return err
	}
	p.SetSequence(seq)
	...
	casOut, writeErr := auth.datastore.WriteCas(auth.LogCtx, p.DocID(), 0, p.Cas(), p, 0)
	if writeErr != nil {
		// For timeout errors the write may or may not have succeeded, so the sequence can't be released as unused.
		if !base.IsTimeoutError(writeErr) {
			if releaseErr := sequences.ReleaseSequence(auth.LogCtx, seq); releaseErr != nil {
				base.WarnfCtx(auth.LogCtx, "Error releasing unused sequence %d after a failed resync update of principal %s. Falling back to skipped sequence handling.  Error:%v", seq, base.UD(p.Name()), releaseErr)
			}
		}
		return writeErr
	}
