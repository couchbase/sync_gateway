...
if len(sys.argv) == 1:
    print(
        "You must pass either a URL to a SG database, e.g. http://foo.com/db, or a path to a directory containing 'all_docs.json' and 'changes.json'"
    )
    sys.exit(1)
...
if useHttp:
    ...
    changesPath = (
        httpRoot
        + "/_changes?feed=longpoll&heartbeat=300000&style=all_docs&since=&include_docs=true"
    )
...
for key in sorted(changes_channels.keys()):
    if key == "unassigned":
        print(
            "\r\n " + key + ": " + str(changes_channels[key] - delCount)
        )  # + '(' + str() + ')')
    else:
