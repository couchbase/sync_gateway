...
func TestBlipDeltaSyncPush(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		if base.IsEnterpriseEdition() {
			...
			docRev, cacheErr := rt.GetDatabase().GetRevisionCacheForTest().Get(base.TestCtx(t), "doc1", "1-0335a345b6ffed05707ccc4cbc1b67f4", collection.GetCollectionID(), db.RevCacheOmitBody, db.RevCacheOmitDelta)
