...
//Simulate two SG instances to test rev_cache side effects
func TestDocDeletionFromChannelTwoSG(t *testing.T) {
	// See https://github.com/couchbase/couchbase-lite-ios/issues/59
	// base.LogKeys["Changes"] = true
	// base.LogKeys["Cache"] = true
	...
	rt2 := RestTester{RestTesterBucket: rt1.RestTesterBucket, SyncFn: `function(doc) {channel(doc.channel)}`}
	...
	// Create a doc Alice can see:
	response := rt1.Send(request("PUT", "/db/alpha", `{"channel":"zero"}`))

	// Check the _changes feed:
	rt1.ServerContext().Database("db").WaitForPendingChanges()
	var changes struct {
		Results []db.ChangeEntry
	}
	response = rt1.Send(requestByUser("GET", "/db/_changes", "", "alice"))
	log.Printf("_changes looks like: %s", response.Body.Bytes())
	json.Unmarshal(response.Body.Bytes(), &changes)
	assert.Equals(t, len(changes.Results), 1)
	since := changes.Results[0].Seq
	...
	// Delete the document:
	assertStatus(t, rt1.Send(request("DELETE", "/db/alpha?rev="+rev1, "")), 200)

	// Get the updates from the _changes feed:
	rt1.WaitForPendingChanges()
	response = rt1.Send(requestByUser("GET", fmt.Sprintf("/db/_changes?since=%s", since),
		"", "alice"))
	log.Printf("_changes looks like: %s", response.Body.Bytes())
	changes.Results = nil
	json.Unmarshal(response.Body.Bytes(), &changes)
	...
	// Now get the deleted revision:
	response = rt1.Send(requestByUser("GET", "/db/alpha?rev="+rev2, "", "alice"))
	assert.Equals(t, response.Code, 200)
	log.Printf("Deletion looks like: %s", response.Body.Bytes())
	var docBody db.Body
	json.Unmarshal(response.Body.Bytes(), &docBody)
	...
	// Access without deletion revID shouldn't be allowed (since doc is not in Alice's channels):
	response = rt1.Send(requestByUser("GET", "/db/alpha", "", "alice"))
	assert.Equals(t, response.Code, 403)

	// A bogus rev ID should return a 404:
	response = rt1.Send(requestByUser("GET", "/db/alpha?rev=bogus", "", "alice"))
	assert.Equals(t, response.Code, 404)

	// Get the old revision, which should still be accessible:
	response = rt1.Send(requestByUser("GET", "/db/alpha?rev="+rev1, "", "alice"))
	assert.Equals(t, response.Code, 200)

	// Resurect document, but in another channel not visible to alice
	assertStatus(t, rt1.Send(request("PUT", "/db/alpha?rev="+rev2, `{"channel":"not-zero"}`)), 201)

	// Now get the deleted revision on SG instance1
	response = rt1.Send(requestByUser("GET", "/db/alpha?rev="+rev2, "", "alice"))
	assert.Equals(t, response.Code, 200)
	log.Printf("Deletion looks like: %s", response.Body.Bytes())
	var deletedDocBody db.Body
	...
	assert.Equals(t, response.Code, 200)
	log.Printf("Deletion looks like: %s", response.Body.Bytes())
	var deletedDocBody2 db.Body
	json.Unmarshal(response.Body.Bytes(), &deletedDocBody2)
	assert.DeepEquals(t, deletedDocBody2, db.Body{"_id": "alpha", "_rev": rev2, "_deleted": true})
