...
// GetInactiveDatabase attempts to load the database and return it's DatabaseContext by fetching the config from the cluster.
...
func newBaseImportOptions(config *DbConfig) *db.ImportOptions {
	...
	if config.ImportPartitions == nil {
		importOptions.ImportPartitions = uint16(base.DefaultImportPartitions)
	} else {
