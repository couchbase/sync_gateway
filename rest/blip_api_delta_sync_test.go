...
func TestBlipDeltaSyncPush(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		message := btcRunner.PushUnsolicitedRev(client.id, docID, &deletedVersion, []byte(`{"undelete":true}`))
		...
		if base.IsEnterpriseEdition() {
			...
			require.NoError(t, err)
			require.Contains(t, string(body), "Can't use delta. Found tombstone for doc")
		} else {
