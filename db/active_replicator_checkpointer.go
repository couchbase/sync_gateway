...
func (c *Checkpointer) upgradeFromSGR1Checkpoint() (sgr1CheckpointSeq *SequenceID) {
	...
	if err != nil || sgr1CheckpointSeq == nil {
...
// if the checkpoint does not exist, returns empty sequence and rev.
func (c *Checkpointer) getRemoteSGR1Checkpoint() (seq *SequenceID, rev string, err error) {
	...
	case http.StatusOK:
		...
		seq, err := ParsePlainSequenceID(body.LastSequence)
		if err != nil {
			return nil, "", err
		}
		return &seq, body.RevID, nil
...
// if the checkpoint does not exist, returns empty sequence and rev.
func (c *Checkpointer) getLocalSGR1Checkpoint() (seq *SequenceID, rev string, err error) {
	...
	sequence, err := ParsePlainSequenceID(body["lastSequence"].(string))
	if err != nil {
		return nil, "", err
	}
	rev, _ = body[BodyRev].(string)

	return &sequence, rev, nil
}
