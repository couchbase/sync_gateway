...
func printEndofTestStatsFile(ctx context.Context, dbContext *db.DatabaseContext) {
	...
	_, _ = fmt.Fprintf(os.Stdout, "%d,", dbStats.Cache().DeprecatedSkippedSeqLen.Value())
	_, _ = fmt.Fprintf(os.Stdout, "%d,", dbStats.Cache().DeprecatedSkippedSeqCap.Value())
...
func csvStats(ctx context.Context, dbContext *db.DatabaseContext) {
	...
	for {
		...
		case <-ticker.C:
			...
			_, _ = fmt.Fprintf(os.Stderr, "%d,", dbStats.Cache().DeprecatedSkippedSeqLen.Value())
			_, _ = fmt.Fprintf(os.Stderr, "%d,", dbStats.Cache().DeprecatedSkippedSeqCap.Value())
