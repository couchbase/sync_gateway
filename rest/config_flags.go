...
func registerConfigFlags(config *StartupConfig, fs *flag.FlagSet) map[string]configFlag {
	return map[string]configFlag{
		...
		"replicator.max_concurrent_replications": {&config.Replicator.MaxConcurrentReplications, fs.Uint("replicator.max_concurrent_replications", 0, "Maximum number of concurrent replications allowed")},
