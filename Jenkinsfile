            ...
            parallel {
                ...
                stage('LiteCore') {
                    stages {
                        ...
                        stage('against EE') {
                            steps {
                                sh 'docker run --net=host --rm -v /root/.ssh/id_rsa:/root/.ssh/id_rsa -v `pwd`/sync_gateway_ee-linux:/sync_gateway -v output.out:/output.out jrascagneres/sglitecoretest:latest'
