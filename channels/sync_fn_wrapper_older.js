function() { var syncFn = %s;
// (put the interpolated sync fn on line 1 so the line numbers in any syntax error will match)
...
// This is the "older" one, for use with Otto. Any changes should be synced with the other.
...

    {
        ...
        var gUserCtx;		    // object with keys .name, .roles, .channels
        var gShouldValidate;	// if false, 'require...' functions are no-ops
        var gResult;			// object to be returned, with keys .channels, .access
        ...
        function match(what /*:string | string[]*/, against /*:string*/) {
            if (Array.isArray(what)) {
                return _.contains(what, against);
            } else {
                return (what == against);
            }
        }
        ...
        function matchInObject(what /*:string | string[]*/, against /*:object*/) {
            if (Array.isArray(what)) {
                for (var i = 0; i < what.length; ++i) {
                    if (against.hasOwnProperty(what[i])) return true;
                }
                return false;
            } else {
        ...
        function matchInArray(what /*:string | string[]*/, against /*:string[]*/) {
            if (Array.isArray(what)) {
                for (var i = 0; i < what.length; ++i) {
                    if (_.contains(against, what[i])) return true;
                }
                return false;
            } else {
                return _.contains(against, what);
            }
        }
        ...
        function appendForUser(map, user, items) {
            if (Array.isArray(user)) {
                for (var i = 0; i < user.length; ++i) {
                    _appendForUser(map, user[i], items);
                }
            } else {
        ...
        requireUser = function(nameOrNames) {
        ...
        channel = function() {
            appendTo(_.toArray(arguments), gResult.channels)
        }

        access = function(user, channels) {
            if (gResult.access === undefined) gResult.access = {}
            appendForUser(gResult.access, user, channels);
        }

        role = function(user, roles) {
        ...
        reject = function(status, message) {
