...
func (bh *blipHandler) handleGetCheckpoint(rq *blip.Message) error {
	...
	value, err := bh.collection.GetLocal(CheckpointDocIDPrefix + client)
	if err != nil {
		return err
	}
	if value == nil {
...
func (bh *blipHandler) handleSetCheckpoint(rq *blip.Message) error {
	...
	revID, err := bh.collection.PutLocal(CheckpointDocIDPrefix+checkpointMessage.client(), checkpoint)
	if err != nil {
		return err
	}
