...
func (db *DatabaseCollectionWithUser) GetDelta(ctx context.Context, docID, fromRevID, toRevID string, versionVectorEnabled bool) (delta *RevisionDelta, redactedRev *DocumentRevision, err error) {
	...
	if versionVectorEnabled {
		fromRev, err = CreateVersionFromString(fromRevID)
		if err != nil {
			return nil, nil, err
		}
	...
	if fromRevision.BodyBytes != nil {
		...
		cv, err := CreateVersionFromString(toRevID)
		if err != nil {
			return nil, nil, err
		}
		// pending backward compatibility we need get with rev function here for revtree clients
...
func (db *DatabaseCollectionWithUser) backupAncestorRevs(ctx context.Context, doc *Document, newDoc *Document) bool {
	...
	for {
		if ancestorRevId = doc.History.getParent(ancestorRevId); ancestorRevId == "" {
			...
			backupRev = db.backupRevisionJSON(ctx, doc.ID, newDoc.RevID, "", newBodyBytes, nil, doc.Attachments)
			return backupRev
	...
	backupRev = db.backupRevisionJSON(ctx, doc.ID, newDoc.RevID, doc.HLV.GetCurrentVersionString(), newBodyBytes, json, doc.Attachments)
...
func (db *DatabaseCollectionWithUser) updateAndReturnDoc(ctx context.Context, docid string, allowImport bool, expiry *uint32, opts *sgbucket.MutateInOptions, docUpdateEvent DocUpdateType, existingDoc *sgbucket.BucketDocument, callback updateAndReturnDocCallback) (doc *Document, newRevID string, err error) {
	...
	if !db.UseXattrs() {
		...
		_, err = db.dataStore.Update(key, initialExpiry, func(currentValue []byte) (raw []byte, syncFuncExpiry *uint32, isDelete bool, err error) {
			...
			syncFuncExpiry, newRevID, storedDoc, oldBodyJSON, unusedSequences, changedAccessPrincipals, changedRoleAccessUsers, createNewRevIDSkipped, _, _, err = db.documentUpdateFunc(ctx, docExists, doc, allowImport, docSequence, unusedSequences, callback, expiry, docUpdateEvent)
			if err != nil {
				return
			}
	...
	if db.UseXattrs() || upgradeInProgress {
		...
		casOut, err = db.dataStore.WriteUpdateWithXattr(ctx, key, base.SyncXattrName, db.userXattrKey(), initialExpiry, existingDoc, opts, func(currentValue []byte, currentXattr []byte, currentUserXattr []byte, cas uint64) (raw []byte, rawXattr []byte, deleteDoc bool, syncFuncExpiry *uint32, updatedSpec []sgbucket.MacroExpansionSpec, err error) {
			...
			syncFuncExpiry, newRevID, storedDoc, oldBodyJSON, unusedSequences, changedAccessPrincipals, changedRoleAccessUsers, createNewRevIDSkipped, revokedChannelsRequiringExpansion, backupRev, err = db.documentUpdateFunc(ctx, docExists, doc, allowImport, docSequence, unusedSequences, callback, expiry, docUpdateEvent)
			if err != nil {
				return
			}
