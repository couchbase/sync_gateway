...
func TestDeleteDatabasePointingAtSameBucketPersistent(t *testing.T) {
   ...
   "num_index_replicas": 0 }`
	...
	_, fetchDb1DestErr := base.FetchDestFactory(base.DestKey("db1", scopeName, collectionNames, base.ImportShardedDCPFeedType))
	assert.Equal(t, base.ErrNotFound, fetchDb1DestErr)
	_, fetchDb2DestErr := base.FetchDestFactory(base.DestKey("db2", scopeName, collectionNames, base.ImportShardedDCPFeedType))
