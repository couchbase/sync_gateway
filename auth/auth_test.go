...
// func setupRevocationScenarios(t *testing.T) (computer, user, role) {
//
// }
...
func TestRevocationScenario1(t *testing.T) {
	...
	// Get Principals / Rebuild
	aliceUserPrincipal, fooPrincipal = getPrincipals(auth)

	// TODO: User should have access to ch1 via role
	...
	// TODO: User has no access to channel via role
	assert.False(t, aliceUserPrincipal.CanSeeChannel("ch1"))
	userRoleHistory, ok := aliceUserPrincipal.RoleHistory()["foo"]
	require.True(t, ok)
	assert.Equal(t, SomeEntry{Seq: 65, EndSeq: 95}, userRoleHistory.Entries[0])
	channelHistory, ok := fooPrincipal.ChannelHistory()["ch1"]
	require.True(t, ok)
	assert.Equal(t, SomeEntry{Seq: 75, EndSeq: 85}, channelHistory.Entries[0])
}
...
func TestRevocationScenario2(t *testing.T) {
	...
	// TODO: User should no longer have access to doc
	assert.False(t, aliceUserPrincipal.CanSeeChannel("ch1"))
	userRoleHistory, ok := aliceUserPrincipal.RoleHistory()["foo"]
	require.True(t, ok)
	assert.Equal(t, SomeEntry{Seq: 20, EndSeq: 45}, userRoleHistory.Entries[0])
	...
	// TODO: User should have access to ch1
	assert.True(t, aliceUserPrincipal.CanSeeChannel("ch1"))
	...
	require.True(t, ok)
	assert.Equal(t, SomeEntry{Seq: 20, EndSeq: 45}, userRoleHistory.Entries[0])
	assert.Equal(t, SomeEntry{Seq: 65, EndSeq: 95}, userRoleHistory.Entries[1])
...
func TestRevocationScenario3(t *testing.T) {
	...
	channelHistory, ok := fooPrincipal.ChannelHistory()["ch1"]
	require.True(t, ok)
	assert.Equal(t, SomeEntry{Seq: 5, EndSeq: 55}, channelHistory.Entries[0])
	...
	assert.Equal(t, SomeEntry{Seq: 5, EndSeq: 55}, channelHistory.Entries[0])
	assert.Equal(t, SomeEntry{Seq: 75, EndSeq: 85}, channelHistory.Entries[1])
}
...
func TestRevocationScenario7(t *testing.T) {
	...
	// TODO: No changes - doc is still revoked
	assert.False(t, aliceUserPrincipal.CanSeeChannel("ch1"))
}
