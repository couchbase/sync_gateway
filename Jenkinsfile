                ...
                stage('LiteCore') {
                    stages {
                        ...
                        stage('against EE') {
                            steps {
                                sh 'mkdir -p reports'
                                sh 'docker run --net=host --rm -v /root/.ssh/id_rsa_ns-buildbot:/root/.ssh/id_rsa -v `pwd`/sync_gateway_ee-linux:/sync_gateway -v `pwd`/reports/output.out:/output.out jrascagneres/sglitecoretest:latest'
