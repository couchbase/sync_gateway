...
func registerConfigFlags(config *StartupConfig, fs *flag.FlagSet) map[string]configFlag {
	return map[string]configFlag{
		...
		"bootstrap.node_heartbeat_expiry":          {config: &config.Bootstrap.NodeHeartbeatExpiry, flagValue: fs.String("bootstrap.node_heartbeat_expiry", "", "How long since a node's last heartbeat before its cluster compat registry entry is pruned. Floored at 2x config_update_frequency. Default: 60s")},
