...
func TestPostUpgradeIndexesSimple(t *testing.T) {
	...
	database, ctx := db.CreateTestDatabase(t, bucket, db.DatabaseContextOptions{
		CacheOptions: new(db.DefaultCacheOptions()),
		Scopes:       db.GetScopesOptionsDefaultCollectionOnly(t),
	})
	defer database.Close(ctx)

	// create old non-xattr indexes to simulate upgrade from an older SG deployment
	createOldNonXattrIndexes(t, database.MetadataStore)
	n1qlStore, ok := base.AsN1QLStore(database.MetadataStore)
	require.True(t, ok)
	require.NoError(t, n1qlStore.CreateIndex(ctx, "sg_syncDocs_1", "val", "val > 3", &base.N1qlIndexOptions{}))
...
func TestPostUpgradeMultipleCollections(t *testing.T) {
	...
	// create non-xattr indexes to simulate upgrade from an older SG deployment
	createOldNonXattrIndexes(t, ds1, ds2)
	metadataStore, ok := base.AsN1QLStore(database.MetadataStore)
	require.True(t, ok)
	require.NoError(t, metadataStore.CreateIndex(ctx, "sg_users_1", "META().id, name, email, disabled", fmt.Sprintf("META().id LIKE '%s'", db.SyncUserWildcard), &base.N1qlIndexOptions{}))
	require.NoError(t, metadataStore.CreateIndex(ctx, "sg_roles_1", "META().id, name, deleted", fmt.Sprintf("META().id LIKE '%s'", db.SyncRoleWildcard), &base.N1qlIndexOptions{}))
...
func TestRemoveUnusedIndexOnError(t *testing.T) {
	...
	// All previous versions eg. [sg_channels_x1 sg_access_x1]
