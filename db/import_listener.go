...
func (il *importListener) StartImportFeed(bucket base.Bucket, dbStats *base.DbStats, dbContext *DatabaseContext) (err error) {
	...
	if !ok || !base.IsEnterpriseEdition() {
		...
		il.cbgtContext, err = base.StartShardedDCPFeed(dbContext.Name, dbContext.Options.ConfigGroup, dbContext.UUID, dbContext.Heartbeater, bucket, cbStore.GetSpec(), dbContext.Options.ImportOptions.ImportPartitions, dbContext.CfgSG)
		return err
	}
