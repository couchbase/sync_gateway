...
var userFunctionAuthTestConfig = functions.FunctionsConfig{
...
func TestUserFunctions(t *testing.T) {
	rt := rest.NewRestTesterForUserQueries(t, rest.DbConfig{
		UserFunctions: &userFunctionAuthTestConfig,
	})
	if rt == nil {
		return
	}
...
func TestJSFunctionAsGuest(t *testing.T) {
	...
	t.Run("function not configured", func(t *testing.T) {
		...
		rest.AssertStatus(t, response, 401)
...
func testUserFunctionsCommon(t *testing.T, sendReqFn func(string, string, string) *rest.TestResponse) {
...
func testUserFunctionsAsAdmin(t *testing.T, rt *rest.RestTester) {
	testUserFunctionsCommon(t, rt.SendAdminRequest)
...
func testUserFunctionsAsUser(t *testing.T, rt *rest.RestTester) {
	...
	testUserFunctionsCommon(t, sendReqFn)

	// negative cases
	t.Run("function not configured", func(t *testing.T) {
		response := sendReqFn("GET", "/db/_function/xxxx", "")
		rest.AssertStatus(t, response, 403)
	})
...
var userN1QLFunctionsAuthTestConfig = functions.FunctionsConfig{
	Definitions: functions.FunctionsDefs{
		"square": &functions.FunctionConfig{
			Type:  "query",
...
func TestUserN1QLQueries(t *testing.T) {
	...
	request, err := json.Marshal(userN1QLFunctionsAuthTestConfig)
...
func TestN1QLFunctionAsGuest(t *testing.T) {
	...
	rt.DatabaseConfig = &rest.DatabaseConfig{
		DbConfig: rest.DbConfig{
			UserFunctions: &userN1QLFunctionsAuthTestConfig,
		},
	}
...
func testUserQueriesCommon(t *testing.T, sendReqFn func(string, string, string) *rest.TestResponse) {
	// positive cases:
	t.Run("commons/passing a param", func(t *testing.T) {
		response := sendReqFn("POST", "/db/_function/square", `{"numero": 16}`)
		rest.AssertStatus(t, response, 200)
...
func testUserQueriesAsAdmin(t *testing.T, rt *rest.RestTester) {
	testUserQueriesCommon(t, rt.SendAdminRequest)
...
func testUserQueriesAsUser(t *testing.T, rt *rest.RestTester) {
	...
	testUserQueriesCommon(t, sendReqFn)
...
func TestFunctionMutability(t *testing.T) {
	userMutabilityFunctionsTestConfig := &functions.FunctionsConfig{
		Definitions: functions.FunctionsDefs{
	...
	request, err := json.Marshal(userMutabilityFunctionsTestConfig)
	assert.NoError(t, err)
	...
	t.Run("Func with mutating True calls another function with a mutating value of False", func(t *testing.T) {
		...
		rest.AssertStatus(t, response, http.StatusForbidden)
	})
...
func TestFunctionTimeout(t *testing.T) {
	userTimeoutFunctionsTestConfig := &functions.FunctionsConfig{
		Definitions: functions.FunctionsDefs{
			"sleep": {
				Type: "javascript",
	...
	rt := rest.NewRestTesterForUserQueries(t, rest.DbConfig{
		UserFunctions: userTimeoutFunctionsTestConfig,
	})
	if rt == nil {
		return
	}
	...
	t.Run("over time limit", func(t *testing.T) {
		response := rt.SendAdminRequest("POST", "/db/_function/sleep", reqBody)
		rest.AssertStatus(t, response, 500)
	})
}
