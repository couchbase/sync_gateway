...
func TestBlipDeltaSyncNewAttachmentPull(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		msg, ok = btcRunner.WaitForBlipRevMessage(client.id, doc1ID, version2)
		assert.True(t, ok)
...
func TestBlipDeltaSyncPullResend(t *testing.T) {
	...
	btcRunner.Run(func(t *testing.T, SupportedBLIPProtocols []string) {
		...
		subProtocol, err := db.ParseSubprotocolString(client.SupportedBLIPProtocols[0])
		require.NoError(t, err)
...
func TestBlipNonDeltaSyncPush(t *testing.T) {
	...
	btcRunner := NewBlipTesterClientRunner(t)
	btcRunner.SkipSubtest[VersionVectorSubtestName] = true // delta sync not implemented for Version Vectors replication 'push'
	const docID = "doc1"
