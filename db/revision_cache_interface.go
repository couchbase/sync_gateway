...
// MutableBody returns an unmarshalled Body which can be modified without affecting the rev cache.
// The returned body does not contain any special properties (i.e. _id, _rev, _attachments, etc)
func (rev *DocumentRevision) MutableBody() (b Body, err error) {
	err = b.Unmarshal(rev.Body)
	if err != nil {
		return nil, err
	}
	return b, nil
...
// Mutable1xBody returns an unmarshalled 1.x style Body which can be modified without affecting the rev cache.
// The returned body contains special properties (i.e. _id, _rev, _attachments, etc.)
func (rev *DocumentRevision) Mutable1xBody(db *Database, requestedHistory Revisions, attachmentsSince []string, showExp bool) (b Body, err error) {
	...
	if showExp && rev.Expiry != nil && !rev.Expiry.IsZero() {
		b["_exp"] = rev.Expiry.Format(time.RFC3339)
	}
	...
	if attachmentsSince != nil && len(rev.Attachments) > 0 {
		...
		b, err = db.loadBodyAttachments(b, minRevpos, rev.DocID)
		if err != nil {
			return nil, err
		}
	}
...
func revCacheLoader(backingStore RevisionCacheBackingStore, id IDAndRev) (bodyBytes []byte, history Revisions, channels base.Set, attachments AttachmentsMeta, expiry *time.Time, deleted bool, err error) {
	var doc *Document
	if doc, err = backingStore.GetDocument(id.DocID, DocUnmarshalAll); doc == nil {
		return bodyBytes, history, channels, attachments, expiry, deleted, err
	}
...
func revCacheLoaderForDocument(backingStore RevisionCacheBackingStore, doc *Document, revid string) (bodyBytes []byte, history Revisions, channels base.Set, attachments AttachmentsMeta, expiry *time.Time, deleted bool, err error) {
	...
	if bodyBytes, err = backingStore.getRevision(doc, revid); err != nil {
		...
		if isRemovalErr != nil {
			return bodyBytes, history, channels, nil, nil, false, isRemovalErr
		}
		if isRemoval {
			return removalBody, removalHistory, removalChannels, nil, nil, false, nil
		} else {
			// If this wasn't a removal, return the original error from getRevision
			return bodyBytes, history, channels, nil, nil, false, err
		}
	}

	validatedHistory, getHistoryErr := doc.History.getHistory(revid)
	if getHistoryErr != nil {
		return bodyBytes, history, channels, nil, nil, false, getHistoryErr
	}
	history = encodeRevisions(validatedHistory)
	channels = doc.History[revid].Channels

	return bodyBytes, history, channels, doc.Attachments, doc.Expiry, doc.History[revid].Deleted, err
}
