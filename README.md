...
Unlike BaseCouch this server supports only a _single_ database in its bucket. The name defaults to "basecouch", but can be changed using the `dbname` command-line flag. Any attempt to access or create other databases will fail with an appropriate HTTP error.
...
The basics are simple: When pulling from ChannelSync using the CouchDB API, configure the replication to use a filter named `basecouch/bychannel`, and a filter parameter `channels` whose value is a comma-separated list of channels to subscribe to. The replication will now only pull documents tagged with those channels.
